#!/bin/bash
echo "[SilverTrade] Starting up..."

# ============================================
# SilverTrade/Cloud Environment Detection & .env Generation
# ============================================
...
if [ -f "$ENV_FILE" ] && [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    echo "[SilverTrade] Using existing .env file"
else
    echo "[SilverTrade] No .env file found or file is empty. Checking for environment variables..."
    
    # Check if we're on Railway/Cloud (HOST_SERVER is the key indicator)
    if [ -n "$HOST_SERVER" ]; then
        echo "[SilverTrade] Environment variables detected. Generating .env file..."
        ...
        if ! touch "$ENV_FILE" 2>/dev/null; then
            echo "[SilverTrade] Cannot write to /app/.env, using /tmp/.env"
            ENV_FILE="/tmp/.env"
        fi
        ...
        cat > "$ENV_FILE" << EOF
# SilverTrade Environment Configuration File
...
DATABASE_URL = '${DATABASE_URL:-sqlite:///db/silvertrade.db}'
...
EOF
        ...
        echo "[SilverTrade] .env file generated at $ENV_FILE"
        echo "[SilverTrade] Configuration: HOST_SERVER=${HOST_SERVER}"
        ...
        if [ "$ENV_FILE" = "/tmp/.env" ]; then
            ...
            echo "[SilverTrade] Linked .env to /app/.env"
        fi
    else
        echo "============================================"
...
PLACEHOLDER_APP_KEY="SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE"
PLACEHOLDER_PEPPER="SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE"
...
============================================================
[SilverTrade] STARTUP BLOCKED — compromised APP_KEY detected
============================================================

Your .env contains the publicly-known sample APP_KEY (and
possibly API_KEY_PEPPER). SilverTrade v1.0.0+ tries to
...
After this, SilverTrade will start cleanly. Existing browser
sessions will need to log in again — APP_KEY rotation
invalidates session cookies, by design.

============================================================
[SilverTrade] DO NOT regenerate API_KEY_PEPPER
============================================================
...
if [ -f "/app/upgrade/migrate_all.py" ]; then
    echo "[SilverTrade] Running database migrations..."
    /app/.venv/bin/python /app/upgrade/migrate_all.py || echo "[SilverTrade] Migration completed (some may have been skipped)"
else
    echo "[SilverTrade] No migrations found, skipping..."
fi

# ============================================
# WEBSOCKET PROXY SERVER
# ============================================
echo "[SilverTrade] Starting WebSocket proxy server on port 8765..."
/app/.venv/bin/python -m websocket_proxy.server &
WEBSOCKET_PID=$!
echo "[SilverTrade] WebSocket proxy server started with PID $WEBSOCKET_PID"

# ============================================
# CLEANUP HANDLER
# ============================================
cleanup() {
    echo "[SilverTrade] Shutting down..."
...
echo "[SilverTrade] Starting application on port ${APP_PORT} with eventlet..."
