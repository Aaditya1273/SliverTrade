...
# SilverTrade AI Docker Installation Script
...
# Start installation
log "Starting SilverTrade AI Docker Installation..." "$GREEN"
log "========================================" "$GREEN"
...
# Get domain name
while true; do
    read -p "Enter your domain name (e.g., demo.silvertrade.ai): " DOMAIN
    if [ -z "$DOMAIN" ]; then
...
# Clone SilverTrade AI repository
...
$SUDO git clone https://github.com/silvertrade/silvertrade.git $INSTALL_PATH
...
$SUDO sed -i "s|SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" .env
$SUDO sed -i "s|SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$API_KEY_PEPPER|g" .env
...
services:
  silvertrade:
    image: silvertrade:latest
    build:
      context: .
      dockerfile: Dockerfile

    container_name: silvertrade-web
...
upstream silvertrade_flask {
    server 127.0.0.1:5000;
    keepalive 64;
}

upstream silvertrade_websocket {
...
server {
    ...
    location = /ws {
        proxy_pass http://silvertrade_websocket;
    ...
    # Socket.IO WebSocket
    location /socket.io/ {
        proxy_pass http://silvertrade_flask/socket.io/;
        proxy_http_version 1.1;
        proxy_read_timeout 86400s;
        proxy_send_timeout 86400s;
        proxy_connect_timeout 60s;
    ...
    location /api/ {
        ...
        proxy_pass http://silvertrade_flask;
        proxy_http_version 1.1;
...
CONTAINER_STATUS=$(sudo docker ps --filter "name=silvertrade-web" --format "{{.Status}}")
...
# Status script
$SUDO tee /usr/local/bin/silvertrade-status > /dev/null << 'EOFSCRIPT'
#!/bin/bash
echo "=========================================="
echo "SilverTrade Status"
echo "=========================================="
echo ""
echo "Container Status:"
sudo docker ps --filter "name=silvertrade-web" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
echo ""
echo "Container Health:"
sudo docker inspect silvertrade-web --format='{{.State.Health.Status}}' 2>/dev/null || echo "Container not found"
echo ""
echo "Recent Logs:"
sudo docker compose -f /opt/silvertrade/docker-compose.yaml logs --tail=30
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/silvertrade-status

# Restart script
$SUDO tee /usr/local/bin/silvertrade-restart > /dev/null << 'EOFSCRIPT'
#!/bin/bash
echo "Restarting SilverTrade AI..."
cd /opt/silvertrade
sudo docker compose restart
sleep 10
echo "Container Status:"
sudo docker ps --filter "name=silvertrade-web"
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/silvertrade-restart

# Logs script
$SUDO tee /usr/local/bin/silvertrade-logs > /dev/null << 'EOFSCRIPT'
#!/bin/bash
cd /opt/silvertrade
sudo docker compose logs -f --tail=100
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/silvertrade-logs

# Backup script
$SUDO tee /usr/local/bin/silvertrade-backup > /dev/null << 'EOFSCRIPT'
#!/bin/bash
BACKUP_DIR="/opt/silvertrade-backups"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="$BACKUP_DIR/silvertrade_backup_$TIMESTAMP.tar.gz"
mkdir -p $BACKUP_DIR
...
sudo docker run --rm -v silvertrade_db:/data -v $TEMP_DIR:/backup alpine tar -czf /backup/db.tar.gz -C /data . 2>/dev/null
sudo docker run --rm -v silvertrade_strategies:/data -v $TEMP_DIR:/backup alpine tar -czf /backup/strategies.tar.gz -C /data . 2>/dev/null
...
cd $BACKUP_DIR
ls -t silvertrade_backup_*.tar.gz 2>/dev/null | tail -n +8 | xargs -r rm
echo "Backup completed!"
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/silvertrade-backup
...
log "\n============================================" "$GREEN"
log "SilverTrade Docker Installation Complete!" "$GREEN"
log "============================================" "$GREEN"
...
log "Broker: $BROKER_NAME" "$BLUE"
log "Installation Path: $INSTALL_PATH" "$BLUE"
log "Container: silvertrade-web" "$BLUE"

log "\nNext Steps:" "$YELLOW"
log "1. Visit https://$DOMAIN to access SilverTrade" "$GREEN"
log "2. Create your admin account and login" "$GREEN"
log "3. Configure your broker settings" "$GREEN"

log "\nUseful Commands:" "$YELLOW"
log "View status:  silvertrade-status" "$BLUE"
log "View logs:    silvertrade-logs" "$BLUE"
log "Restart:      silvertrade-restart" "$BLUE"
log "Backup:       silvertrade-backup" "$BLUE"
