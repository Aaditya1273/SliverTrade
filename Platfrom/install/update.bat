@echo off
REM ============================================================================
REM SilverTrade AI Update Script for Windows
REM ============================================================================
REM
REM Usage: update.bat
REM
REM This script updates SilverTrade AI to the latest version using the UV method.
REM Run from the install\ directory or the silvertrade project root.
REM
REM Prerequisites:
REM   - Python 3.12+
...
if exist "%SCRIPT_DIR%\app.py" (
    ...
    if exist "app.py" (
        ...
        echo [ERROR] Could not find SilverTrade AI directory.
        echo.
        echo Please run this script from:
        echo   - The silvertrade project root directory, OR
        echo   - The install\ directory within silvertrade
        echo.
        pause
        exit /b 1
    )
...
echo [INFO] SilverTrade AI directory: %SILVERTRADE_DIR%
echo.

REM Verify git repository
if not exist "%SILVERTRADE_DIR%\.git" (
    echo [ERROR] Not a git repository: %SILVERTRADE_DIR%
    echo Please ensure SilverTrade AI was installed via git clone.
    echo.
    pause
    exit /b 1
)
...
set "BACKUP_DIR=%SILVERTRADE_DIR%\db\backup_%TIMESTAMP%"
set "BACKUP_COUNT=0"

if exist "%SILVERTRADE_DIR%\db\" (
    md "%BACKUP_DIR%" 2>nul

    for %%f in (silvertrade.db logs.db latency.db sandbox.db) do (
        if exist "%SILVERTRADE_DIR%\db\%%f" (
            copy /y "%SILVERTRADE_DIR%\db\%%f" "%BACKUP_DIR%\%%f" >nul 2>&1
            echo   Backed up: %%f
            set /a BACKUP_COUNT+=1
        )
    )

    if exist "%SILVERTRADE_DIR%\db\historify.duckdb" (
        copy /y "%SILVERTRADE_DIR%\db\historify.duckdb" "%BACKUP_DIR%\historify.duckdb" >nul 2>&1
...
if not exist "%SILVERTRADE_DIR%\.env" (
    echo   [WARNING] No .env file found. Creating from .sample.env...
    if exist "%SILVERTRADE_DIR%\.sample.env" (
        copy /y "%SILVERTRADE_DIR%\.sample.env" "%SILVERTRADE_DIR%\.env" >nul
        ...
        powershell -Command "(Get-Content '%SILVERTRADE_DIR%\.env') -replace 'SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE', '!NEW_APP_KEY!' | Set-Content '%SILVERTRADE_DIR%\.env'"
        powershell -Command "(Get-Content '%SILVERTRADE_DIR%\.env') -replace 'SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE', '!NEW_PEPPER!' | Set-Content '%SILVERTRADE_DIR%\.env'"
...
if exist "%SILVERTRADE_DIR%\upgrade\migrate_all.py" (
...
REM ========================================
if not exist "%SILVERTRADE_DIR%\frontend\dist\" (
    where npm >nul 2>&1
    if not errorlevel 1 (
        echo [OPTIONAL] Building React frontend (dist\ not found)...
        pushd "%SILVERTRADE_DIR%\frontend"
        call npm install
        call npm run build
        if errorlevel 1 (
...
echo.
echo   ========================================
echo   SilverTrade AI Update Complete!
echo   ========================================
echo.
echo   Version:   %CURRENT_COMMIT% -^> %NEW_COMMIT%
echo   Branch:    %CURRENT_BRANCH%
echo   Directory: %SILVERTRADE_DIR%
if exist "%BACKUP_DIR%\" (
