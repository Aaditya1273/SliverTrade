...
# silvertrade Installation Banner
echo -e "${BLUE}"
echo "  ██████╗ ██████╗ ███████╗███╗   ██╗ █████╗ ██╗      ██████╗  ██████╗ "
echo " ██╔═══██╗██╔══██╗██╔════╝████╗  ██║██╔══██╗██║     ██╔════╝ ██╔═══██╗"
echo " ██║   ██║██████╔╝███████╗██╔██╗ ██║███████║██║     ██║  ███╗██║   ██║"
...
# SilverTrade AI Installation and Configuration Script
...
# Start logging
log_message "Starting SilverTrade AI installation log at: $LOG_FILE" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
log_message "SilverTrade Installation Configuration" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
BASE_PATH="/var/python/silvertrade-flask/$DEPLOY_NAME"
SILVERTRADE_PATH="$BASE_PATH/silvertrade"
VENV_PATH="$BASE_PATH/venv"
SOCKET_PATH="$BASE_PATH"
SOCKET_FILE="$SOCKET_PATH/silvertrade.sock"
SERVICE_NAME="silvertrade-$DEPLOY_NAME"
...
log_message "\nStarting SilverTrade AI installation for $DEPLOY_NAME..." "$YELLOW"
...
case "$OS_TYPE" in
    ubuntu | debian | raspbian)
        ...
        # Non-fatal — if nothing sticks we just warn; the rest of silvertrade still installs fine.
...
# Check and handle existing SilverTrade AI installation
handle_existing "$BASE_PATH" "installation directory" "SilverTrade directory for $DEPLOY_NAME"
...
# Clone repository
log_message "\nCloning SilverTrade AI repository..." "$BLUE"
sudo git clone https://github.com/silvertrade/silvertrade.git $SILVERTRADE_PATH
check_status "Failed to clone SilverTrade AI repository"
...
handle_existing "$SILVERTRADE_PATH/.env" "environment file" ".env file"

sudo cp $SILVERTRADE_PATH/.sample.env $SILVERTRADE_PATH/.env
sudo sed -i "s|YOUR_BROKER_API_KEY|$BROKER_API_KEY|g" $SILVERTRADE_PATH/.env
sudo sed -i "s|YOUR_BROKER_API_SECRET|$BROKER_API_SECRET|g" $SILVERTRADE_PATH/.env

# Update market data API credentials if the broker is XTS-based
if is_xts_broker "$BROKER_NAME"; then
    sudo sed -i "s|YOUR_BROKER_MARKET_API_KEY|$BROKER_API_KEY_MARKET|g" $SILVERTRADE_PATH/.env
    sudo sed -i "s|YOUR_BROKER_MARKET_API_SECRET|$BROKER_API_SECRET_MARKET|g" $SILVERTRADE_PATH/.env
fi

sudo sed -i "s|http://127.0.0.1:5000|https://$DOMAIN|g" $SILVERTRADE_PATH/.env
# Explicitly set HOST_SERVER in case the default value didn't match
sudo sed -i "s|HOST_SERVER = '.*'|HOST_SERVER = 'https://$DOMAIN'|g" $SILVERTRADE_PATH/.env
sudo sed -i "s|<broker>|$BROKER_NAME|g" $SILVERTRADE_PATH/.env
sudo sed -i "s|SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" $SILVERTRADE_PATH/.env
sudo sed -i "s|SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$API_KEY_PEPPER|g" $SILVERTRADE_PATH/.env
...
sudo sed -i "s|TRUST_PROXY_HEADERS = 'FALSE'|TRUST_PROXY_HEADERS = 'TRUE'|g" $SILVERTRADE_PATH/.env

# Disable session expiry for crypto brokers (24/7 markets)
if [ "$DISABLE_SESSION_EXPIRY" = "true" ]; then
    sudo sed -i "s|DISABLE_SESSION_EXPIRY = 'false'|DISABLE_SESSION_EXPIRY = 'true'|g" $SILVERTRADE_PATH/.env
    log_message "Session auto-logout disabled for crypto broker" "$GREEN"
fi

# Update WebSocket URL for production
sudo sed -i "s|WEBSOCKET_URL='.*'|WEBSOCKET_URL='wss://$DOMAIN/ws'|g" $SILVERTRADE_PATH/.env
...
handle_existing "/etc/systemd/system/$SERVICE_NAME.service" "systemd service" "SilverTrade service file"

# Create systemd service with unique name
log_message "\nCreating systemd service..." "$BLUE"
sudo tee /etc/systemd/system/$SERVICE_NAME.service > /dev/null << EOL
[Unit]
Description=SilverTrade Gunicorn Daemon ($DEPLOY_NAME)
After=network.target

[Service]
User=$WEB_USER
Group=$WEB_GROUP
WorkingDirectory=$SILVERTRADE_PATH
...
Environment="HOME=$SILVERTRADE_PATH/tmp"
# Environment variables for numba/scipy support
Environment="TMPDIR=$SILVERTRADE_PATH/tmp"
Environment="NUMBA_CACHE_DIR=$SILVERTRADE_PATH/tmp/numba_cache"
Environment="LLVMLITE_TMPDIR=$SILVERTRADE_PATH/tmp"
Environment="MPLCONFIGDIR=$SILVERTRADE_PATH/tmp/matplotlib"
...
sudo chmod -R 755 $SILVERTRADE_PATH
...
log_message "Installation Directory: $SILVERTRADE_PATH" "$BLUE"
log_message "Environment File: $SILVERTRADE_PATH/.env" "$BLUE"
...
log_message "\nNext Steps:" "$YELLOW"
log_message "1. Visit https://$DOMAIN to access your SilverTrade AI instance" "$GREEN"
...
log_message "Restart SilverTrade AI: sudo systemctl restart $SERVICE_NAME" "$BLUE"
