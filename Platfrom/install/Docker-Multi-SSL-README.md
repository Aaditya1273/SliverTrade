# SilverTrade AI Advanced Docker Installation (Multi-Instance & Custom SSL)
...
- **Multiple SilverTrade AI instances** on a single server.
...
```bash
wget https://raw.githubusercontent.com/marketcalls/silvertrade/main/install/install-docker-multi-custom-ssl.sh
...
1.  **Instance Name**:
    ...
    - This allows you to run multiple independent copies of SilverTrade AI side-by-side.
...
```
/opt/
  ├── silvertrade-algo1/       # Instance 1
  │   ├── docker-compose.yaml
  │   └── .env
  └── silvertrade-fyers-bot/   # Instance 2 (Different Broker/Strategy)
      ├── docker-compose.yaml
      └── .env
```
...
3.  **WebSocket 403 Errors**:
    - If you experience disconnects after broker re-login, restart the specific instance:
      ```bash
      cd /opt/silvertrade-INSTANCE_NAME
      docker compose restart
      ```
