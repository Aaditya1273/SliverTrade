...
# SilverTrade AI Update Script
# Updates an existing SilverTrade AI installation to the latest version using the UV method.
...
# Start logging
log_message "Starting SilverTrade AI update log at: $LOG_FILE" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
if [ ${#DEPLOYMENTS[@]} -gt 0 ]; then
    ...
    log_message "Path: $SILVERTRADE_PATH" "$BLUE"
    log_message "Service: $SERVICE_NAME" "$BLUE"
else
    # Check if we're in or near an silvertrade git repo (local development)
    if [ -d ".git" ] && [ -f "app.py" ]; then
        SILVERTRADE_PATH="$(pwd)"
    elif [ -d "$SCRIPT_DIR/../.git" ] && [ -f "$SCRIPT_DIR/../app.py" ]; then
        SILVERTRADE_PATH="$(cd "$SCRIPT_DIR/.." && pwd)"
    else
        log_message "Error: No SilverTrade AI deployment found." "$RED"
        log_message "For server deployments, ensure install.sh was run first." "$YELLOW"
        log_message "For local development, run this script from the silvertrade directory." "$YELLOW"
        exit 1
    fi

    log_message "Detected local development setup at: $SILVERTRADE_PATH" "$GREEN"
fi
...
if [ "$SERVER_MODE" = true ]; then
    CURRENT_COMMIT=$(sudo git -C "$SILVERTRADE_PATH" rev-parse --short HEAD 2>/dev/null || echo "unknown")
    CURRENT_BRANCH=$(sudo git -C "$SILVERTRADE_PATH" branch --show-current 2>/dev/null || echo "main")
else
...
BACKUP_DIR="$SILVERTRADE_PATH/db/backup_${TIMESTAMP}"
BACKUP_COUNT=0

if [ -d "$SILVERTRADE_PATH/db" ]; then
    if [ "$SERVER_MODE" = true ]; then
    ...
    for db_file in silvertrade.db logs.db latency.db sandbox.db; do
        if [ -f "$SILVERTRADE_PATH/db/$db_file" ]; then
            if [ "$SERVER_MODE" = true ]; then
                sudo cp "$SILVERTRADE_PATH/db/$db_file" "$BACKUP_DIR/$db_file"
            else
                cp "$SILVERTRADE_PATH/db/$db_file" "$BACKUP_DIR/$db_file"
            fi
    ...
    if [ -f "$SILVERTRADE_PATH/db/historify.duckdb" ]; then
        if [ "$SERVER_MODE" = true ]; then
            sudo cp "$SILVERTRADE_PATH/db/historify.duckdb" "$BACKUP_DIR/historify.duckdb"
        else
            cp "$SILVERTRADE_PATH/db/historify.duckdb" "$BACKUP_DIR/historify.duckdb"
        fi
...
if [ "$SERVER_MODE" = true ]; then
    LOCAL_CHANGES=$(sudo git -C "$SILVERTRADE_PATH" status --porcelain 2>/dev/null | grep -v "^??" | head -20)
else
...
if [ -n "$LOCAL_CHANGES" ]; then
    ...
    if [ "$SERVER_MODE" = true ]; then
        sudo git -C "$SILVERTRADE_PATH" stash push -m "auto-stash before update $TIMESTAMP"
    else
...
# Pull latest code
if [ "$SERVER_MODE" = true ]; then
    sudo git -C "$SILVERTRADE_PATH" pull origin "$CURRENT_BRANCH"
else
...
# Get new commit hash
if [ "$SERVER_MODE" = true ]; then
    NEW_COMMIT=$(sudo git -C "$SILVERTRADE_PATH" rev-parse --short HEAD 2>/dev/null || echo "unknown")
else
...
if [ -f "$SILVERTRADE_PATH/.env" ] && [ -f "$SILVERTRADE_PATH/.sample.env" ]; then
    # Extract variable names from both files and compare
    SAMPLE_VARS=$(grep -oP "^[A-Z_][A-Z_0-9]+ *=" "$SILVERTRADE_PATH/.sample.env" 2>/dev/null | sed 's/ *=$//' | sort -u)
    CURRENT_VARS=$(grep -oP "^[A-Z_][A-Z_0-9]+ *=" "$SILVERTRADE_PATH/.env" 2>/dev/null | sed 's/ *=$//' | sort -u)
...
elif [ ! -f "$SILVERTRADE_PATH/.env" ]; then
    log_message "Warning: No .env file found. Creating from .sample.env..." "$YELLOW"
    if [ "$SERVER_MODE" = true ]; then
        sudo cp "$SILVERTRADE_PATH/.sample.env" "$SILVERTRADE_PATH/.env"
    else
        cp "$SILVERTRADE_PATH/.sample.env" "$SILVERTRADE_PATH/.env"
    fi
    ...
    if [ "$SERVER_MODE" = true ]; then
        sudo sed -i "s|SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$NEW_APP_KEY|g" "$SILVERTRADE_PATH/.env"
        sudo sed -i "s|SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$NEW_PEPPER|g" "$SILVERTRADE_PATH/.env"
        sudo chmod 600 "$SILVERTRADE_PATH/.env"
    else
        sed -i.bak "s|SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$NEW_APP_KEY|g" "$SILVERTRADE_PATH/.env" && rm -f "$SILVERTRADE_PATH/.env.bak"
        sed -i.bak "s|SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$NEW_PEPPER|g" "$SILVERTRADE_PATH/.env" && rm -f "$SILVERTRADE_PATH/.env.bak"
        chmod 600 "$SILVERTRADE_PATH/.env"
    fi
    log_message "Generated fresh APP_KEY and API_KEY_PEPPER in $SILVERTRADE_PATH/.env" "$GREEN"
    log_message "Please edit $SILVERTRADE_PATH/.env with your broker credentials and settings." "$RED"
fi
...
if [ -f "$SILVERTRADE_PATH/.env" ]; then
    ...
    if [ "$SERVER_MODE" = true ]; then
        ENV_PERMS=$(stat -c '%a' "$SILVERTRADE_PATH/.env" 2>/dev/null || stat -f '%Lp' "$SILVERTRADE_PATH/.env" 2>/dev/null)
    ...
    if ! grep -q "^TRUST_PROXY_HEADERS" "$SILVERTRADE_PATH/.env"; then
        # Detect nginx in front of silvertrade: any sites-enabled/ or conf.d/
        # config that mentions a unix-socket proxy_pass or the deployment name.
        BEHIND_NGINX="false"
        if [ -d /etc/nginx/sites-enabled ]; then
            if find /etc/nginx/sites-enabled -type f -o -type l 2>/dev/null | xargs grep -l "unix:.*\.sock\|silvertrade\|gunicorn" 2>/dev/null | head -1 | grep -q .; then
                BEHIND_NGINX="true"
            fi
        fi
        if [ "$BEHIND_NGINX" = "false" ] && [ -d /etc/nginx/conf.d ]; then
            if find /etc/nginx/conf.d -type f -name "*.conf" 2>/dev/null | xargs grep -l "unix:.*\.sock\|silvertrade\|gunicorn" 2>/dev/null | head -1 | grep -q .; then
                BEHIND_NGINX="true"
            fi
        fi
        if [ "$BEHIND_NGINX" = "true" ]; then
            echo "" | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            echo "# Auto-added by update.sh — nginx reverse proxy detected." | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            echo "TRUST_PROXY_HEADERS = 'TRUE'" | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            log_message "Added TRUST_PROXY_HEADERS=TRUE to .env (nginx reverse proxy detected)" "$GREEN"
        else
            echo "" | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            echo "# Auto-added by update.sh — set to TRUE only if behind a reverse proxy" | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            echo "# that strips client-supplied X-Forwarded-For / CF-Connecting-IP / X-Real-IP." | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
            echo "TRUST_PROXY_HEADERS = 'FALSE'" | sudo tee -a "$SILVERTRADE_PATH/.env" >/dev/null
...
if [ "$SERVER_MODE" = true ]; then
    ...
    sudo $UV_CMD pip install --python "$VENV_PATH/bin/python" -r "$SILVERTRADE_PATH/requirements-nginx.txt"
...
if [ "$SERVER_MODE" = true ]; then
    ...
    sudo mkdir -p "$SILVERTRADE_PATH/db"
    sudo mkdir -p "$SILVERTRADE_PATH/tmp/numba_cache"
    sudo mkdir -p "$SILVERTRADE_PATH/tmp/matplotlib"
    sudo mkdir -p "$SILVERTRADE_PATH/strategies/scripts"
    sudo mkdir -p "$SILVERTRADE_PATH/strategies/examples"
    sudo mkdir -p "$SILVERTRADE_PATH/log/strategies"
    sudo mkdir -p "$SILVERTRADE_PATH/keys"
    sudo chown -R "$WEB_USER:$WEB_GROUP" "$SILVERTRADE_PATH"
    sudo chmod 700 "$SILVERTRADE_PATH/keys"

    log_message "Permissions set successfully" "$GREEN"

    # Run migrations as the web user (database files are owned by web user)
    if [ -f "$SILVERTRADE_PATH/upgrade/migrate_all.py" ]; then
        log_message "Running database migrations..." "$BLUE"
        sudo -u "$WEB_USER" bash -c "source $VENV_PATH/bin/activate && cd $SILVERTRADE_PATH && python upgrade/migrate_all.py" 2>&1 | tee -a "$LOG_FILE"
        if [ ${PIPESTATUS[0]} -ne 0 ]; then
            log_message "Retrying migrations with elevated permissions..." "$YELLOW"
            sudo bash -c "source $VENV_PATH/bin/activate && cd $SILVERTRADE_PATH && python upgrade/migrate_all.py" 2>&1 | tee -a "$LOG_FILE"
        fi
...
if [ "$SERVER_MODE" = true ]; then
    ...
    # Start the SilverTrade AI service
    ...
    if [ ! -d "$SILVERTRADE_PATH/frontend/dist" ]; then
        if command -v npm >/dev/null 2>&1; then
            log_message "Building React frontend (dist/ not found)..." "$BLUE"
            cd "$SILVERTRADE_PATH/frontend"
...
# ============================================
log_message "\n========================================" "$GREEN"
log_message "  SilverTrade AI Update Summary" "$GREEN"
log_message "========================================" "$GREEN"
