# SilverTrade AI Installation Guide
...
# Or using curl
curl -O https://raw.githubusercontent.com/marketcalls/silvertrade/main/install/install.sh
...
- Set up the SilverTrade AI application with unique deployment name
...
- Each deployment gets a unique service name (e.g., silvertrade-yourdomain-broker)
...
1. **Check Service Status**
   ...
   sudo systemctl status silvertrade-fyers-yourdomain-fyers
   
   # Example for Zerodha deployment
   sudo systemctl status silvertrade-zerodha-yourdomain-zerodha
   ```
...
2. **Application Not Starting**
   ...
   sudo journalctl -u silvertrade-fyers-yourdomain-fyers    # View logs
   sudo systemctl restart silvertrade-fyers-yourdomain-fyers # Restart service
   
   # Example 2: Zerodha deployment on zerodha.yourdomain.com
   sudo journalctl -u silvertrade-zerodha-yourdomain-zerodha # View logs
   sudo systemctl restart silvertrade-zerodha-yourdomain-zerodha # Restart service
   ```
...
1. **SELinux blocking Nginx**
   ...
   sudo semanage fcontext -a -t httpd_sys_rw_content_t "/var/python/silvertrade-flask(/.*)?"
   sudo restorecon -Rv /var/python/silvertrade-flask
   ```
...
1. **Service Management Examples**
   ...
   # Example outputs:
   # silvertrade-fyers-yourdomain-fyers.service    loaded active running
   # silvertrade-zerodha-yourdomain-zerodha.service loaded active running
   
   # Restart specific deployment
   sudo systemctl restart silvertrade-fyers-yourdomain-fyers
...
2. **Log Management Examples**
   ...
   sudo journalctl -f -u silvertrade-fyers-yourdomain-fyers
   
   # View last 100 lines of Zerodha deployment logs
   sudo journalctl -n 100 -u silvertrade-zerodha-yourdomain-zerodha
   
   # View logs since last hour for specific deployment
   sudo journalctl --since "1 hour ago" -u silvertrade-fyers-yourdomain-fyers
   ```
...
4. **Installation Directory Examples**
   ...
   ls -l /var/python/silvertrade-flask/
   
   # Example structure:
   # /var/python/silvertrade-flask/fyers-yourdomain-fyers/
   # /var/python/silvertrade-flask/zerodha-yourdomain-zerodha/
   
   # Check specific deployment files
   ls -l /var/python/silvertrade-flask/fyers-yourdomain-fyers/
   ```
...
2. **SELinux (RHEL-based systems)**
   - The installation script **automatically configures SELinux** for SilverTrade
...
- Check the [GitHub repository](https://github.com/silvertrade/silvertrade)
