...
# SilverTrade AI Multi-Instance Installation Banner
echo -e "${BLUE}"
echo "  ██████╗ ██████╗ ███████╗███╗   ██╗ █████╗ ██╗      ██████╗  ██████╗ "
echo " ██╔═══██╗██╔══██╗██╔════╝████╗  ██║██╔══██╗██║     ██╔════╝ ██╔═══██╗"
echo " ██║   ██║██████╔╝███████╗██╔██╗ ██║███████║██║     ██║  ███╗██║   ██║"
...
# Start logging
log_message "Starting SilverTrade AI Multi-Instance installation" "$BLUE"
...
while true; do
    read -p "How many SilverTrade AI instances do you want to set up? " INSTANCES
...
log_message "Setting up $INSTANCES SilverTrade AI instances" "$GREEN"

# Base configuration
BASE_DIR="/var/python/silvertrade-flask"
REPO_URL="https://github.com/silvertrade/silvertrade.git"
...
# Non-fatal — if nothing sticks we warn; the rest of silvertrade still installs fine.
...
for ((i=1; i<=INSTANCES; i++)); do
    ...
    INSTANCE_DIR="$BASE_DIR/silvertrade$i"
    VENV_PATH="$INSTANCE_DIR/venv"
    SOCKET_FILE="$INSTANCE_DIR/silvertrade.sock"
    SERVICE_NAME="silvertrade$i"
    ...
    DB_PATH="sqlite:///db/silvertrade${i}.db"
    ...
    sudo sed -i "s|SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" "$ENV_FILE"
    sudo sed -i "s|SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$API_KEY_PEPPER|g" "$ENV_FILE"
...
[Unit]
Description=SilverTrade Instance $i ($DOMAIN - $BROKER)
...
for ((i=1; i<=INSTANCES; i++)); do
    ...
    log_message "  Service: silvertrade$i" "$BLUE"
    log_message "  Directory: $BASE_DIR/silvertrade$i" "$BLUE"
done

log_message "\n📚 USEFUL COMMANDS:" "$YELLOW"
log_message "View all services: systemctl list-units 'silvertrade*'" "$BLUE"
log_message "Restart instance: sudo systemctl restart silvertrade<N>" "$BLUE"
log_message "View logs: sudo journalctl -u silvertrade<N> -f" "$BLUE"
log_message "Check status: sudo systemctl status silvertrade<N>" "$BLUE"
