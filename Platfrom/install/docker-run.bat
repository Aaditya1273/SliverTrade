@echo off
REM ============================================================================
REM SilverTrade AI Docker Runner for Windows
REM ============================================================================
REM
REM Quick Start (2 commands):
REM   1. Download: curl.exe -O https://raw.githubusercontent.com/marketcalls/silvertrade/main/install/docker-run.bat
REM   2. Run:      docker-run.bat
REM
REM Commands:
REM   start    - Start SilverTrade AI container (default, runs setup if needed)
...
REM Configuration
set IMAGE=marketcalls/silvertrade:latest
set CONTAINER=silvertrade
set ENV_FILE=.env
set SAMPLE_ENV_URL=https://raw.githubusercontent.com/marketcalls/silvertrade/main/.sample.env
REM Use the directory where the script is located
set SILVERTRADE_DIR=%~dp0
REM Remove trailing backslash
if "%SILVERTRADE_DIR:~-1%"=="\" set SILVERTRADE_DIR=%SILVERTRADE_DIR:~0,-1%
set SETUP_FAILED=0
...
echo.
echo   ========================================
echo        SilverTrade AI Docker Runner
...
:setup
echo [INFO] Setting up SilverTrade AI in %SILVERTRADE_DIR%...
echo.

REM Create db directory
if not exist "%SILVERTRADE_DIR%\db\" (
    echo [INFO] Creating database directory...
    md "%SILVERTRADE_DIR%\db" 2>nul
    if errorlevel 1 (
...
if exist "%SILVERTRADE_DIR%\%ENV_FILE%" (
    echo [WARNING] .env file already exists at %SILVERTRADE_DIR%\%ENV_FILE%
...
if errorlevel 1 (
    ...
    powershell -Command "Invoke-WebRequest -Uri '%SAMPLE_ENV_URL%' -OutFile '%SILVERTRADE_DIR%\%ENV_FILE%'" 2>nul
) else (
    curl.exe -sL "%SAMPLE_ENV_URL%" -o "%SILVERTRADE_DIR%\%ENV_FILE%" 2>nul
)

REM Check if download succeeded
if not exist "%SILVERTRADE_DIR%\%ENV_FILE%" (
    echo [ERROR] Failed to download configuration template!
    echo Please check your internet connection.
    echo.
    echo Manual setup:
    echo   1. Download .sample.env from https://github.com/silvertrade/silvertrade
    echo   2. Save it as %SILVERTRADE_DIR%\.env
    echo   3. Run this script again
    set SETUP_FAILED=1
    goto setup_end
)
...
powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'SILVERTRADE_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE', '%APP_KEY%' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'SILVERTRADE_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE', '%API_KEY_PEPPER%' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"
...
powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY = ''YOUR_BROKER_API_KEY''', 'BROKER_API_KEY = ''%BROKER_API_KEY%''' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET = ''YOUR_BROKER_API_SECRET''', 'BROKER_API_SECRET = ''%BROKER_API_SECRET%''' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"

REM Update redirect URL with broker name (replace <broker> placeholder)
powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace '<broker>', '%BROKER_NAME%' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"

REM Update XTS market data credentials if applicable
if "%IS_XTS%"=="1" (
    powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY_MARKET = ''YOUR_BROKER_MARKET_API_KEY''', 'BROKER_API_KEY_MARKET = ''!BROKER_API_KEY_MARKET!''' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"
    powershell -Command "(Get-Content '%SILVERTRADE_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET_MARKET = ''YOUR_BROKER_MARKET_API_SECRET''', 'BROKER_API_SECRET_MARKET = ''!BROKER_API_SECRET_MARKET!''' | Set-Content '%SILVERTRADE_DIR%\%ENV_FILE%'"
)
...
echo   Data directory: %SILVERTRADE_DIR%
echo   Config file:    %SILVERTRADE_DIR%\%ENV_FILE%
echo   Database:       %SILVERTRADE_DIR%\db\
echo   Strategies:     %SILVERTRADE_DIR%\strategies\
echo   Logs:           %SILVERTRADE_DIR%\log\
echo.
echo   Redirect URL for broker portal:
echo   http://127.0.0.1:5000/%BROKER_NAME%/callback
echo.
echo   Documentation: https://docs.silvertrade.ai
echo.
set /p OPEN_ENV="Open .env in Notepad for review? (y/n): "
if /i "%OPEN_ENV%"=="y" (
    start notepad "%SILVERTRADE_DIR%\%ENV_FILE%"
)
echo.
echo [OK] Setup complete! Run 'docker-run.bat start' to launch SilverTrade AI.

:setup_end
exit /b %SETUP_FAILED%

:start
echo [INFO] Starting SilverTrade AI...
echo.
...
if not exist "%SILVERTRADE_DIR%\%ENV_FILE%" (
    ...
    if errorlevel 1 (
        echo.
        echo [ERROR] Setup failed. Cannot start SilverTrade AI.
        echo Please fix the issues above and try again.
        goto end
    )
    echo.
    echo [INFO] Starting SilverTrade AI after setup...
    echo.
)
...
if not exist "%SILVERTRADE_DIR%\strategies\" (
    echo [INFO] Creating strategies directory...
    md "%SILVERTRADE_DIR%\strategies" 2>nul
    md "%SILVERTRADE_DIR%\strategies\scripts" 2>nul
    md "%SILVERTRADE_DIR%\strategies\examples" 2>nul
)
if not exist "%SILVERTRADE_DIR%\log\" (
    echo [INFO] Creating log directory...
    md "%SILVERTRADE_DIR%\log" 2>nul
    md "%SILVERTRADE_DIR%\log\strategies" 2>nul
)
if not exist "%SILVERTRADE_DIR%\keys\" (
    echo [INFO] Creating keys directory...
    md "%SILVERTRADE_DIR%\keys" 2>nul
)
if not exist "%SILVERTRADE_DIR%\tmp\" (
    echo [INFO] Creating temp directory...
    md "%SILVERTRADE_DIR%\tmp" 2>nul
)
...
REM See: https://github.com/silvertrade/silvertrade/issues/822
...
docker run -d ^
    ...
    -e "TZ=Asia/Kolkata" ^
    -v "%SILVERTRADE_DIR%\db:/app/db" ^
    -v "%SILVERTRADE_DIR%\strategies:/app/strategies" ^
    -v "%SILVERTRADE_DIR%\log:/app/log" ^
    -v "%SILVERTRADE_DIR%\keys:/app/keys" ^
    -v "%SILVERTRADE_DIR%\tmp:/app/tmp" ^
    -v "%SILVERTRADE_DIR%\.env:/app/.env" ^
...
if errorlevel 1 (
    ...
    echo   3. Check .env file: %SILVERTRADE_DIR%\%ENV_FILE%
    echo.
    goto end
)

echo.
echo [SUCCESS] SilverTrade AI started successfully!
echo.
echo   ========================================
...
echo     docker-run.bat stop     - Stop SilverTrade
echo     docker-run.bat restart  - Restart SilverTrade
echo.
goto end

:stop
echo [INFO] Stopping SilverTrade AI...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] SilverTrade AI stopped.
goto end

:restart
echo [INFO] Restarting SilverTrade AI...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] SilverTrade AI stopped.
echo.
...
if errorlevel 1 (
    echo [STATUS] SilverTrade AI is NOT running.
) else (
    echo [STATUS] SilverTrade AI is running.
    echo.
    echo   Web UI: http://127.0.0.1:5000
)
echo.
...
echo.
echo Commands:
echo   start    Start SilverTrade AI (runs setup if needed, default)
...
echo      curl.exe -O https://raw.githubusercontent.com/marketcalls/silvertrade/main/install/docker-run.bat
echo   3. Run: docker-run.bat
echo.
echo Data Location: %SILVERTRADE_DIR%
echo   - Config:     %SILVERTRADE_DIR%\.env
echo   - Database:   %SILVERTRADE_DIR%\db\
echo   - Strategies: %SILVERTRADE_DIR%\strategies\
echo   - Logs:       %SILVERTRADE_DIR%\log\
echo.
