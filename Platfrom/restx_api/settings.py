...
settings_model = api.model("Settings", {
    ...
    "min_signal_confidence": fields.Integer(description="Minimum signal confidence to execute (50-95)"),
    "max_open_positions": fields.Integer(description="Maximum concurrent open positions"),
    "daily_loss_limit_pct": fields.Integer(description="Stop trading if daily P&L drops below -X%"),
    "auto_execute": fields.Boolean(description="Auto-execute signals above confidence threshold"),
})
...
class UserSettingsResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return make_response(jsonify({"status": "error", "message": "Failed to get settings"}), 500)
    ...
    def post(self):
        ...
        try:
            data = request.json
            if not data:
                return make_response(jsonify({"status": "error", "message": "No settings data provided"}), 400)
            ...
            if risk is not None and (risk < 0.5 or risk > 5):
                return make_response(jsonify({
                    "status": "error",
                    "message": "Risk per trade must be between 0.5% and 5%"
                }), 400)
            ...
            return make_response(jsonify({"status": "error", "message": "Failed to save settings"}), 500)
