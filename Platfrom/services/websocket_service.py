...
def get_user_api_key(username: str) -> str | None:
    ...
    try:
        ...
        if not api_key:
            logger.warning(
                f"No API key found for user {username}. User needs to generate an API key first."
            )
        return api_key
...
def get_websocket_connection(username: str) -> tuple[bool, WebSocketClient | None, str | None]:
    """
...
def get_websocket_status(
    ...
    try:
        ...
        if not success:
            return (
                False,
                {"status": "error", "message": error, "connected": False, "authenticated": False},
                200,
            )  # Return 200 even for disconnected state
...
def get_websocket_subscriptions(
    ...
    try:
        ...
        if not success:
            return False, {"status": "error", "message": error, "subscriptions": []}, 200
...
def subscribe_to_symbols(
    ...
    try:
        ...
        if not success:
            return False, {"status": "error", "message": error}, 503  # Service Unavailable
        ...
        if result.get("status") == "success":
            ...
            return (
                False,
                {"status": "error", "message": result.get("message", "Subscription failed")},
                400,
            )
...
def unsubscribe_from_symbols(
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            return (
                False,
                {"status": "error", "message": result.get("message", "Unsubscription failed")},
                400,
            )
...
def unsubscribe_all(username: str, broker: str) -> tuple[bool, dict[str, Any], int]:
    ...
    try:
        ...
        if result.get("status") == "success":
            return (
                True,
                {"status": "success", "message": "Unsubscribed from all symbols", "broker": broker},
                200,
            )
        else:
            return (
...
def get_market_data(
    ...
    try:
        ...
        if not success:
            return False, {"status": "error", "message": error, "data": {}}, 200

        # Get market data
        market_data = client.get_market_data(symbol, exchange)

        return True, {"status": "success", "data": market_data, "timestamp": int(time.time())}, 200
