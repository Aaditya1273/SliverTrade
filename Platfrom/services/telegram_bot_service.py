...
class TelegramBotService:
    """Service class for managing Telegram bot operations with SilverTrade AI SDK integration"""
    ...
    def __init__(self):
        ...
        self.sdk_clients = {}  # Cache for SilverTrade AI SDK clients per user
        self._stop_event = original_threading.Event()  # Thread-safe stop signal

    def _get_sdk_client(self, telegram_id: int) -> silvertrade_api | None:
        """Get or create SilverTrade AI SDK client for a user"""
        from silvertrade_sdk import api as silvertrade_api
        ...
        try:
            ...
            # Create SDK client
            client = silvertrade_api(api_key=api_key, host=host_url)
    ...
    def _render_plotly_png(self, fig) -> bytes:
        ...
        t = original_threading.Thread(
            target=_worker, daemon=True, name="silvertrade-kaleido-render"
        )
        t.start()
        t.join()
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            await update.message.reply_text(
                f"Welcome to SilverTrade AI Bot, {user.first_name}! 🚀\n\n"
                "To get started, link your SilverTrade AI account:\n"
...
*Account Management:*
/link `<api_key> <host_url>` - Link your SilverTrade AI account
...
"""
    ...
    async def cmd_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            test_client = silvertrade_api(api_key=api_key, host=host_url)
            ...
            if test_response and test_response.get("status") == "success":
                ...
                # Get the actual SilverTrade AI username from the API key
                silvertrade_username = None
                try:
                    silvertrade_username = get_username_by_apikey(api_key)
                    logger.info(f"API key lookup returned: '{silvertrade_username}'")
                except Exception as e:
                ...
                if not silvertrade_username and test_response.get("data"):
                    # Some brokers return username in the funds response
                    data = test_response.get("data", {})
                    if isinstance(data, dict):
                        silvertrade_username = (
                            data.get("username") or data.get("user_id") or data.get("client_id")
                        )
                        if silvertrade_username:
                            logger.info(f"Got username from funds response: {silvertrade_username}")

                # Log for debugging
                logger.info(
                    f"Linking Telegram user {user.id} (@{user.username}) with SilverTrade AI username: '{silvertrade_username}'"
                )

                # If we still can't get username, DON'T use telegram username with @
                # Use a proper fallback
                if not silvertrade_username:
                    # Try to get from session or use telegram ID
                    silvertrade_username = f"user_{user.id}"
                    logger.warning(
                        f"Could not get SilverTrade AI username, using fallback: {silvertrade_username}"
                    )
                else:
                    logger.info(f"Successfully retrieved SilverTrade AI username: {silvertrade_username}")
                ...
                create_or_update_telegram_user(
                    telegram_id=user.id,
                    username=silvertrade_username,  # Use the actual SilverTrade AI username
                ...
                logger.info(f"Database updated - Username stored as: {silvertrade_username}")
    ...
    async def cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            # Get display name (prefer telegram_username, fallback to silvertrade_username)
            display_name = (
                telegram_user.get("telegram_username")
                or telegram_user.get("silvertrade_username")
                or "N/A"
            )
            ...
            await update.message.reply_text(
                "❌ No linked account found.\nUse /link to connect your SilverTrade AI account.",
                parse_mode=ParseMode.MARKDOWN,
            )
    ...
    async def cmd_orderbook(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await update.message.reply_text("❌ Failed to connect to SilverTrade")
            return
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if callback_data == "confirm_closeall":
            ...
            client = self._get_sdk_client(user.id)
            if not client:
                await query.edit_message_text("❌ Failed to connect to SilverTrade")
                return
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await context.bot.send_message(chat_id=chat_id, text="❌ Failed to connect to SilverTrade")
            return
    ...
    async def broadcast_message(self, message: str, filters: dict = None) -> tuple[int, int]:
        ...
        try:
            ...
            if filters:
                ...
                if filters.get("silvertrade_username"):
                    users = [
                        u
                        for u in users
                        if u.get("silvertrade_username") == filters["silvertrade_username"]
                    ]
