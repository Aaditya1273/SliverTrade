...
class BillingService:
    ...
    def create_checkout_session(self, user_id: int, plan: str, interval: str = "month") -> Optional[str]:
        ...
        try:
            ...
            if user_id:
                ...
                if user:
                    ...
                    items_data = items.get("data", []) if isinstance(items, dict) else (items.data if hasattr(items, 'data') else [])
                    ...
                    logger.info(f"Activated {plan} subscription for user {user_id} (price: {price_id})")
        except Exception as e:
