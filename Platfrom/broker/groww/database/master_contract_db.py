...
# Functions for symbol format conversion between SilverTrade AI and Groww formats
def format_silvertrade_to_groww_symbol(symbol, exchange):
    """
    Convert SilverTrade AI symbol format to Groww symbol format

    Args:
        symbol (str): Symbol in SilverTrade AI format (e.g., AARTIIND29MAY25630CE)
        exchange (str): Exchange code (NSE, BSE, NFO, etc.)

    Returns:
        str: Symbol in Groww format (e.g., "AARTIIND 29MAY25 630 CE")
    """
    logger.info(f"Converting symbol from SilverTrade AI to Groww format: {{symbol}}, {exchange}")
...
def format_groww_to_silvertrade_symbol(groww_symbol, exchange):
    """
    Convert Groww symbol format to SilverTrade AI symbol format
    ...
    Returns:
        str: Symbol in SilverTrade AI format (e.g., "AARTIIND29MAY25630CE")
    """
    logger.info(f"Converting symbol from Groww to SilverTrade AI format: {{groww_symbol}}, {exchange}")
    ...
    if exchange == "NFO":
        ...
        # If already in SilverTrade AI format (no spaces), return as is
        ...
        if len(parts) >= 4 and parts[-1] in ("CE", "PE"):
            ...
            # Combine into SilverTrade AI format: [BaseSymbol][ExpirationDate][StrikePrice][OptionType]
            # Example: AARTIIND29MAY25630CE
            silvertrade_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
            logger.info(f"Converted to SilverTrade AI format: {silvertrade_symbol}")
            return silvertrade_symbol
        ...
        # For futures
            ...
            # Combine into SilverTrade AI format: [BaseSymbol][ExpirationDate]FUT
            # Example: NIFTY29MAY25FUT
            silvertrade_symbol = f"{base_symbol}{date_str}FUT"
            logger.info(f"Converted to SilverTrade AI format: {silvertrade_symbol}")
            return silvertrade_symbol
        ...
        # Handle case where option type might be missing
            ...
            if is_date:
                try:
                    ...
                    silvertrade_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
                    logger.info(
                        f"Converted to SilverTrade AI format (assumed {{option_type}}): {silvertrade_symbol}"
                    )
                    return silvertrade_symbol
                except ValueError:
...
def find_symbol_by_token(token, exchange):
    ...
    Returns:
        str: Symbol in SilverTrade AI format, or None if not found
    """
...
def find_token_by_symbol(symbol, exchange):
    ...
    Args:
        symbol (str): Symbol in either SilverTrade AI or Groww format
        exchange (str): Exchange code
    ...
    if exchange == "NFO":
        # Try with SilverTrade AI format if it was in Groww format
        silvertrade_symbol = format_groww_to_silvertrade_symbol(symbol, exchange)
        if silvertrade_symbol != symbol:
            result = (
                db_session.query(SymToken)
                .filter_by(symbol=silvertrade_symbol, exchange=exchange)
                .first()
            )
            if result:
                return result.token

        # Try with Groww format if it was in SilverTrade AI format
...
def process_groww_data(path):
    ...
    try:
        ...
        # Replace specific index symbols with standardized SilverTrade AI names
        # Mapping Groww index symbols -> SilverTrade AI standard symbols
        # (from symbol_Silvertrade.md documentation)
        symbol_replacements = {
            # NSE Index symbols
            "NIFTYJR": "NIFTYNXT50",
...
def master_contract_download():
    ...
    try:
        ...
        # Step 5: Add SilverTrade AI symbols where needed (vectorized - remove spaces from brsymbol)
        # For NFO options with spaces in brsymbol, the SilverTrade AI format is just the symbol without spaces
        nfo_space_mask = (
