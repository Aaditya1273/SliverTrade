...
class GrowwExchangeMapper:
    """Maps SilverTrade AI exchange codes to Groww exchange/segment format"""

    # Mapping from SilverTrade AI exchange to Groww exchange and segment
    EXCHANGE_MAP = {
    ...
    @classmethod
    def get_exchange(cls, silvertrade_exchange: str) -> str:
        """
        Get Groww exchange from SilverTrade AI exchange code

        Args:
            silvertrade_exchange: SilverTrade AI exchange code (e.g., 'NSE', 'NFO')

        Returns:
            str: Groww exchange code
        """
        mapping = cls.EXCHANGE_MAP.get(silvertrade_exchange, {})
        return mapping.get("exchange", silvertrade_exchange)

    @classmethod
    def get_segment(cls, silvertrade_exchange: str) -> str:
        """
        Get Groww segment from SilverTrade AI exchange code
    ...
    @classmethod
    def get_exchange_segment(cls, silvertrade_exchange: str) -> tuple:
        """
        Get both exchange and segment from SilverTrade AI exchange code

        Args:
            silvertrade_exchange: SilverTrade AI exchange code

        Returns:
            tuple: (exchange, segment)
        """
        mapping = cls.EXCHANGE_MAP.get(silvertrade_exchange, {})
        return mapping.get("exchange", silvertrade_exchange), mapping.get("segment", "CASH")
