...
def map_order_data(order_data):
    """
    Processes and modifies order data from Groww format to SilverTrade AI format.

    Parameters:
    - order_data: A dictionary with either 'data' key or raw Groww API response with 'order_list'.

    Returns:
    - The modified order_data with standardized fields in SilverTrade AI format.
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Convert broker symbol to SilverTrade AI format
        silvertrade_symbol = broker_symbol
        ...
        if silvertrade_symbol == broker_symbol and " " in broker_symbol:
            try:
                # Look up in database
        ...
        mapped_order = {
            ...
            "symbol": silvertrade_symbol,  # Using the converted SilverTrade AI format symbol
        ...
        # Map status to SilverTrade AI format
        status_map = {
            "NEW": "open",
            "ACKED": "open",
        ...
        # Map product type to SilverTrade AI format
...
def transform_order_data(orders):
    """
    Transform order data from Groww API format to SilverTrade AI standard format

    Args:
        orders (dict): Order data from Groww API

    Returns:
        list: Transformed orders in SilverTrade AI format for orderbook.py
    """
    ...
    for i, order in enumerate(orders_to_process):
        ...
        # Get proper SilverTrade AI symbol from database using token lookup
        token = None
        ...
        # If we have a token or brsymbol (tradingsymbol/trading_symbol), look up the SilverTrade AI symbol from the database
        try:
            from database.token_db import get_oa_symbol

            # Try to get the SilverTrade AI symbol using the token if available
            if token:
            ...
            # If token lookup failed or token wasn't available, try by broker symbol
                ...
                # First check if we already have the SilverTrade AI symbol
                if exchange == "NFO" and (
                    broker_symbol.endswith("CE") or broker_symbol.endswith("PE")
                ):
                    # Query the database to find the SilverTrade AI symbol for this broker symbol
                    from broker.groww.database.master_contract_db import SymToken, db_session
                    ...
                    with db_session() as session:
                        ...
                        if record and record.symbol:
                            ...
                            logger.info(
                                f"Found SilverTrade AI symbol in database: {broker_symbol} -> {symbol}"
                            )
        except Exception as e:
            logger.error(f"Error looking up SilverTrade AI symbol from database: {e}")
        ...
        # Map order type to SilverTrade AI format
        ...
        # For NFO instruments, ensure the symbol is in SilverTrade AI format (AARTIIND29MAY25630CE)
        exchange = order.get("exchange", "NSE")
        if exchange == "NFO" and " " in symbol:
            try:
                ...
                if silvertrade_symbol:
                    ...
                    # Use SilverTrade AI symbol format for display
                    symbol = silvertrade_symbol
                    logger.info(f"Transformed order symbol for UI: {broker_symbol} -> {symbol}")
            except Exception as e:
                logger.error(f"Error converting order symbol format: {e}")

        # Create transformed order in SilverTrade AI format
        transformed_order = {
            "symbol": symbol,  # Now guaranteed to be in SilverTrade AI format
            "exchange": order.get("exchange", "NSE"),
    ...
    # Final check to ensure all symbols are in SilverTrade AI format using database lookups
        ...
        if order.get("exchange") == "NFO" and "symbol" in order and order["symbol"]:
            ...
            if token:
                try:
                    ...
                    silvertrade_symbol = get_oa_symbol(token, order.get("exchange", "NSE"))
                    if silvertrade_symbol:
                        order["symbol"] = silvertrade_symbol
                        logger.info(f"Final token lookup: {symbol} -> {silvertrade_symbol}")
                        continue
                except Exception as e:
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        ...
        # Try to get SilverTrade AI symbol from database
        try:
            from database.token_db import get_oa_symbol
