...
def convert_date(date_str):
    """
    Convert date format to SilverTrade expiry column format: DD-MMM-YY (with hyphens).
    Example: '19MAR2024' -> '19-MAR-24' or '2024-03-19' -> '19-MAR-24'

    SilverTrade Expiry Column Format: DD-MMM-YY (e.g., 28-MAR-24, 25-APR-24)
...
def fetch_and_process_mstock_indices():
    ...
    try:
        ...
        # Standardize NSE index symbols that differ from SilverTrade standard format
...
def process_mstock_json(json_data):
    """
    Processes the MStock JSON data to fit the SilverTrade database schema.
    ...
    # MStock API returns NSE/BSE for currency, but SilverTrade uses CDS/BCD
    # -------------------------------------------------------------------
    ...
    # Convert expiry dates to SilverTrade format (DD-MMM-YY)
    ...
    try:
        ...
        if bse_index_tokens:
            ...
            # Normalize BSE Index symbols to SilverTrade standard format
    ...
    # -------------------------------------------------------------------
    # Format F&O Symbols (SilverTrade Standard)
    ...
    # -------------------------------------------------------------------
    # Normalize instrumenttype to SilverTrade standard (match Angel format)
