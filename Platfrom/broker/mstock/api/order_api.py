...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    Args:
        tradingsymbol: SilverTrade format symbol
...
def place_smartorder_api(data, auth):
    ...
    Args:
        data: SilverTrade order data with position_size
        auth: Authentication token
...
def modify_order(data, auth):
    ...
    Args:
        data: SilverTrade modify order data with fields:
            - orderid: Order ID to modify
            - symbol: SilverTrade symbol
...
def cancel_all_orders_api(data, auth):
    ...
    response = request_with_circuit_breaker(
        "POST", "https://api.mstock.trade/openapi/typeb/orders/cancelall", headers=headers
    )
