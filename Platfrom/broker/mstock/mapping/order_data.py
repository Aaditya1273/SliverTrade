...
def map_broker_exchange_to_silvertrade(broker_exchange, instrumenttype=""):
    """
    Maps mStock broker exchange to SilverTrade AI exchange format.
    mStock returns NSE for both equity and derivatives,
    but SilverTrade AI uses NFO for derivatives.
    ...
    Returns:
    - SilverTrade AI exchange format (e.g., 'NFO' for NSE derivatives)
    """
...
def map_order_data(order_data):
    ...
    Converts broker symbols to SilverTrade AI symbols and maps product types.
    ...
    if order_data:
        for order in order_data:
            ...
            # Get SilverTrade AI symbol from database using symboltoken
            ...
            # Map product types to SilverTrade AI format
...
def transform_order_data(orders):
    """
    Transforms mStock Type B order data to SilverTrade AI format.

    Parameters:
    - orders: List of order dictionaries from mStock Type B API

    Returns:
    - List of orders in SilverTrade AI format
    """
    ...
    for order in orders:
        ...
        # Normalize status to SilverTrade AI format (lowercase)
        ...
        transformed_order = {
            ...
            "exchange": map_broker_exchange_to_silvertrade(
...
def map_trade_data(trade_data):
    ...
    for trade in trade_data:
        ...
        if exchange:
            # First try: Get SilverTrade AI symbol using symboltoken
            if symboltoken:
                symbol_from_db = get_symbol(symboltoken, lookup_exchange)
                if symbol_from_db:
...
def transform_tradebook_data(tradebook_data):
    """
    Transforms mStock Type B tradebook data to SilverTrade AI format.

    Parameters:
    - tradebook_data: List of trade dictionaries from mStock Type B API

    Returns:
    - List of trades in SilverTrade AI format
    """
    transformed_data = []
...
def transform_positions_data(positions_data):
    """
    Transforms mStock Type B positions data to SilverTrade AI format.

    Parameters:
    - positions_data: List of position dictionaries (already mapped by map_position_data)

    Returns:
    - List of positions in SilverTrade AI format
    """
    transformed_data = []
...
def transform_holdings_data(holdings_data):
    """
    Transforms mStock Type B holdings data to SilverTrade AI format.
    ...
    Returns:
    - List of holdings in SilverTrade AI format
    """
    transformed_data = []
...
def map_portfolio_data(portfolio_data):
    ...
    # Convert to expected format for compatibility with SilverTrade
    if isinstance(data, list):
        ...
        for holding in data:
            ...
            # Get SilverTrade AI symbol from broker symbol
            if symbol:
