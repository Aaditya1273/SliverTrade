...
def map_exchange(exchange):
    """
    Maps SilverTrade AI exchange names to Motilal Oswal exchange names.

    SilverTrade AI uses: NSE, BSE, NFO, CDS, MCX, BFO
...
def reverse_map_exchange(exchange):
    """
    Reverse maps Motilal Oswal exchange names to SilverTrade AI exchange names.
    """
...
def transform_data(data, token, auth_token=None):
    """
    Transforms the SilverTrade AI API request structure to Motilal Oswal expected structure.
    ...
    silvertrade_exchange = data["exchange"]
    motilal_exchange = map_exchange(silvertrade_exchange)
    ...
    if pricetype == "MARKET":
        logger.info(
            ...
            f"Exchange={silvertrade_exchange}, Action={action}"
        )
        try:
            if auth_token:
                ...
                quote_data = broker_data.get_quotes(data["symbol"], silvertrade_exchange)
                ltp = float(quote_data.get("ltp", 0)) if quote_data else 0

                if ltp > 0:
                    instrument_type = get_instrument_type_from_symbol(data["symbol"])
                    tick_size = None
                    symbol_info = get_symbol_info(data["symbol"], silvertrade_exchange)
    ...
    transformed = {
        ...
        "producttype": map_product_type(
            data["product"], silvertrade_exchange
        ),  # Pass SilverTrade AI exchange for context
...
def transform_modify_order_data(data, token, lastmodifiedtime, qtytradedtoday):
    ...
    Args:
        data: SilverTrade AI modify order request data
...
def map_order_type(pricetype):
    """
    Maps SilverTrade AI pricetype to Motilal Oswal order type.
...
def map_product_type(product, exchange=None):
    """
    Maps SilverTrade AI product type to Motilal Oswal product type based on exchange.
    ...
    Args:
        product: SilverTrade AI product type (CNC, MIS, NRML)
        exchange: SilverTrade AI exchange name (NSE, BSE, NFO, CDS, MCX, BFO)
    """
    # Determine if this is a cash segment or derivative segment
    # Using SilverTrade AI exchange names
...
def reverse_map_product_type(product, exchange=None):
    """
    Reverse maps Motilal Oswal product type to SilverTrade AI product type.
    Context:
