...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    # Convert Trading Symbol from SilverTrade Format to Broker Format Before Search in OpenPosition
    tradingsymbol = get_br_symbol(tradingsymbol, exchange)
    # Map exchange from SilverTrade format to Motilal format for comparison
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response.get("status") == "SUCCESS":
        ...
        for position in positions_response["data"]:
            ...
            # Convert Motilal exchange to SilverTrade exchange for symbol lookup
            motilal_exchange = position["exchange"]
            silvertrade_exchange = reverse_map_exchange(motilal_exchange)

            # Get silvertrade symbol to send to placeorder function
            symbol = get_symbol(position["symboltoken"], silvertrade_exchange)
            logger.info(f"The Symbol is {symbol}")

            if not symbol:
                logger.error(
                    f"Symbol not found for token {position['symboltoken']} and exchange {silvertrade_exchange}"
                )
                continue
            ...
            place_order_payload = {
                ...
                "symbol": symbol,
                "action": action,
                "exchange": silvertrade_exchange,  # Use SilverTrade exchange format
                "pricetype": "MARKET",
                "product": reverse_map_product_type(position["productname"], silvertrade_exchange),
