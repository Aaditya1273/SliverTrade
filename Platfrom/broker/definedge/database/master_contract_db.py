...
def process_definedge_nse_csv(path):
    ...
    try:
        ...
        # Map DefinedGe NSE columns to SilverTrade AI schema
        # Assuming DefinedGe uses standard format: Symbol, Token, Name, etc.
        processed_df = pd.DataFrame()

        if "Symbol" in df.columns:
            processed_df["symbol"] = df["Symbol"] + "-EQ"  # SilverTrade AI format
...
def process_definedge_allmaster_csv(path):
    ...
    try:
        ...
        # Format symbols according to SilverTrade AI standard
        ...
        # Step 3: Explicit rename map for symbols whose cleaned form differs from SilverTrade AI standard
