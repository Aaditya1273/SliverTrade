...
class DhanExchangeMapper:
    """Maps between SilverTrade AI exchange names and Dhan exchange codes"""

    # SilverTrade AI to Dhan exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE_EQ",
        "BSE": "BSE_EQ",
        "NFO": "NSE_FNO",
    ...
    # Dhan exchange segment codes (numeric) to SilverTrade AI exchange mapping
    ...
    # Reverse mappings
    DHAN_TO_SILVERTRADE = {v: k for k, v in EXCHANGE_MAP.items()}
    EXCHANGE_TO_SEGMENT = {v: k for k, v in SEGMENT_TO_EXCHANGE.items()}

    @classmethod
    def get_dhan_exchange(cls, silvertrade_exchange: str) -> str | None:
        """Convert SilverTrade AI exchange to Dhan exchange format"""
        return cls.EXCHANGE_MAP.get(silvertrade_exchange)

    @classmethod
    def get_silvertrade_exchange(cls, dhan_exchange: str) -> str | None:
        """Convert Dhan exchange to SilverTrade AI exchange format"""
        return cls.DHAN_TO_SILVERTRADE.get(dhan_exchange)

    @classmethod
    def get_exchange_from_segment(cls, segment_code: int) -> str | None:
        """Convert Dhan exchange segment code to SilverTrade AI exchange"""
    ...
    def get_segment_from_exchange(cls, exchange: str) -> int | None:
        """Convert SilverTrade AI exchange to Dhan exchange segment code"""
