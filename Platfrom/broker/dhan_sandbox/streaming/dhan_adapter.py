"""
Fixed Dhan WebSocket adapter for SilverTrade AI.
...
from .dhan_mapping import get_dhan_exchange, get_silvertrade_exchange
...
class DhanWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Implements SilverTrade WebSocket proxy interface with proper mode mapping.
    """
    ...
    def __init__(self):
        ...
        # Extended mode mapping to handle all possible SilverTrade AI modes
        self.mode_map = {
            # Standard SilverTrade AI modes
    ...
    def subscribe(
        ...
        try:
            ...
            # Map SilverTrade AI mode to Dhan mode with explicit handling of all possible modes
            if mode == 1:
                # Standard LTP mode
                ...
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(
                    f"Mapped SilverTrade AI mode {mode} (DEPTH/FULL) to Dhan mode '{dhan_mode}'"
                )

            # Add symbol to subscription tracking
            with self.lock:
                self.subscribed_symbols[symbol] = {
                    "exchange": exchange,  # Store original SilverTrade AI exchange
            ...
            # Map SilverTrade AI exchange to Dhan exchange code
    ...
    def _on_ticks(self, ticks: list[dict[str, Any]]):
        ...
        try:
            ...
            for tick in ticks:
                ...
                # Convert to SilverTrade AI format for data field
                ...
                # Normalize tick format to SilverTrade AI standard
    ...
    def _generate_topic(self, symbol: str, exchange: str, mode_str: str) -> str:
        ...
        Args:
            ...
            exchange: The exchange code in SilverTrade AI format
    ...
    def _normalize_tick(self, tick: dict[str, Any]) -> dict[str, Any]:
        """
        Normalize Dhan tick data to SilverTrade AI format.
        ...
        try:
            ...
            # Ensure exchange is in SilverTrade AI format
            exchange = tick.get("exchange")
            if exchange:
                silvertrade_exchange = get_silvertrade_exchange(exchange)
            else:
    ...
    def validate_tick_parsing(self, ticker_symbols=None, timeout=30):
        ...
        try:
            ...
            original_callbacks = {}
            for mode in [1, 2, 3, 4, 5]:  # SilverTrade AI modes
