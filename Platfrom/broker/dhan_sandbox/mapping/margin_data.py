...
def transform_margin_position(position, client_id):
    """
    Transform a single SilverTrade AI margin position to Dhan Sandbox margin format.

    Note: Dhan Sandbox margin calculator API accepts only one order at a time, not a batch.

    Args:
        position: Position in SilverTrade AI format
...
def map_product_type_for_margin(product):
    """
    Maps SilverTrade AI product type to Dhan Sandbox product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan Sandbox margin response to SilverTrade AI standard format.
    ...
    try:
        ...
        # Return standardized format with SilverTrade-compatible aliases.
        return {
            "status": "success",
            "data": {
...
def parse_multi_margin_response(response_data):
    """
    Parse Dhan Sandbox /v2/margincalculator/multi response to SilverTrade AI standard format.
