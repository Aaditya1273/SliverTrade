...
class ZerodhaWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Properly implements SilverTrade WebSocket proxy interface with correct topic formatting.
    """
    ...
    def _transform_tick(self, tick: dict) -> dict | None:
        """Transform Zerodha tick to SilverTrade format with index support"""
        try:
            token = tick.get("instrument_token")
            if not token:
                return None
