# Zerodha GTT payload transforms (SilverTrade ⇄ Kite).
...
def transform_place_gtt(data):
    """Transform an SilverTrade AI flat place-GTT payload into Kite's `{type, condition, orders}`.
...
def transform_modify_gtt(data):
    """Transform an SilverTrade AI modify-GTT payload (flat shape) into Kite's body.
...
def map_gtt_book(gtt_data):
    """Normalise Kite's GET /gtt/triggers response into a SilverTrade-shaped list.
    ...
    Kite tradingsymbol back to SilverTrade's symbol.
    """
