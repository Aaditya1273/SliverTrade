        ...
        try:
            if method == "GET":
                response = request_with_circuit_breaker(
                    "GET", f"{base_url}{endpoint}", headers=headers, timeout=30.0
                )
            else:
                response = request_with_circuit_breaker(
                    "POST", f"{base_url}{endpoint}", headers=headers, content=payload, timeout=30.0
                )
...
def get_open_positionss(tradingsymbol, exchange, product, auth):
    ...
    # Convert Trading Symbol from SilverTrade Format to Broker Format (Token ID)
