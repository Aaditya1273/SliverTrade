"""
Fyers Data Mapping
Maps Fyers HSM data to SilverTrade AI format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to SilverTrade AI format
    """
    ...
    def map_to_silvertrade_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to SilverTrade AI LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            SilverTrade AI LTP format dict or None if mapping fails
        """
        try:
            ...
            # Map to SilverTrade AI LTP format
            silvertrade_data = {
            ...
            return silvertrade_data

        except Exception as e:
            logger.debug(f"Error mapping LTP data: {e}")
            return None

    def map_to_silvertrade_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to SilverTrade AI Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            SilverTrade AI Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
            ...
            # Map to SilverTrade AI Quote format
            silvertrade_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_to_silvertrade_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to SilverTrade AI Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            SilverTrade AI Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to SilverTrade AI Depth format (matching other brokers)
            silvertrade_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_tbt_depth_to_silvertrade(
        self, ticker: str, tbt_depth: dict[str, Any], symbol: str, exchange: str
    ) -> dict[str, Any] | None:
        """
        Map Fyers TBT 50-level depth data to SilverTrade AI Depth format

        Args:
            ticker: Original Fyers ticker (e.g., 'NSE:RELIANCE-EQ')
            tbt_depth: Depth data from TBT WebSocket
            symbol: SilverTrade AI symbol name
            exchange: SilverTrade AI exchange name
        ...
        try:
            ...
            # Map to SilverTrade AI Depth format
            silvertrade_data = {
                "symbol": symbol,
                "exchange": exchange,
                "token": ticker,
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic SilverTrade AI Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate SilverTrade AI format based on requested type
        ...
        Returns:
            Mapped SilverTrade AI data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_silvertrade_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_silvertrade_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_silvertrade_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from SilverTrade AI format
