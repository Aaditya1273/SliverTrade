...
def transform_margin_positions(positions):
    """
    Transform SilverTrade AI margin position format to IndMoney margin format.
...
def map_segment(exchange):
    """
    Maps SilverTrade AI exchange to IndMoney segment.

    SilverTrade AI: NSE, BSE, NFO, BFO, CDS, BCD, MCX
...
def map_exchange_type(exchange):
    """
    Maps SilverTrade AI exchange to IndMoney exchange format.
...
def map_product_type(product):
    """
    Maps SilverTrade AI product type to IndMoney product type.

    SilverTrade AI: CNC, NRML, MIS
...
def parse_margin_response(response_data):
    """
    Parse IndMoney margin calculator response to SilverTrade AI standard format.

    Args:
        response_data: Raw response from IndMoney margin calculator API

    Returns:
        Standardized margin response matching SilverTrade AI format
    """
    try:
        ...
        # Extract only the three required values as per SilverTrade AI API specification
        total_margin = data.get("total_margin", 0)
        span_margin = data.get("span_margin", 0)
        exposure_margin = data.get("exposure_margin", 0)

        # Return standardized format matching SilverTrade AI API specification
