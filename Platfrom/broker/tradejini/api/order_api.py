...
def get_order_book(auth):
    ...
    Returns:
        dict: Order book data in SilverTrade format
    """
    try:
        # Get API key from environment
        api_key = os.getenv("BROKER_API_SECRET")
        ...
        # Transform response data to SilverTrade format
        ...
        if response_data["s"] == "ok":
            ...
            # Transform each order to SilverTrade format
            transformed_orders = []
            for order in response_data["d"]:
                # logger.debug(f"[DEBUG] get_order_book - Processing order: {order}")
                try:
                    # Get SilverTrade symbol using symbol and exchange
                    silvertrade_symbol = get_oa_symbol(order["sym"]["id"], order["sym"]["exch"])
                    # print(f"[DEBUG] get_order_book - SilverTrade symbol lookup for symbol {order['sym']['sym']}: {silvertrade_symbol}")

                    transformed_order = {
                        "stat": "Ok",  # SilverTrade expects 'stat' field
                        "data": {
                            "tradingsymbol": silvertrade_symbol
                            if silvertrade_symbol
                            else order["sym"][
                                "sym"
                            ],  # Fallback to Tradejini symbol if SilverTrade not found
...
def get_trade_book(auth):
    ...
    Returns:
        dict: Trade book data in SilverTrade format {'data': [...], 'status': 'success'}
    """
    try:
        # Get API key from environment
        api_key = os.getenv("BROKER_API_SECRET")
        ...
        # Transform trades directly to SilverTrade format
        transformed_trades = []
        for trade in trades_data:
            try:
                ...
                # Get SilverTrade symbol
                silvertrade_symbol = None
                try:
                    silvertrade_symbol = get_oa_symbol(symbol.get("id", ""), symbol.get("exch", ""))
                except Exception as e:
                ...
                # Create transformed trade - match SilverTrade format exactly
                # Determine the symbol to use (SilverTrade symbol if available)
                final_symbol = ""
                if silvertrade_symbol:
                    final_symbol = silvertrade_symbol
                else:
                    # Fallback to exchange symbol if SilverTrade symbol isn't available
                ...
                transformed_trade = {
                    ...
                    "symbol": final_symbol,  # Using SilverTrade symbol here
...
def get_positions(auth):
    ...
    Returns:
        dict: Positions data in SilverTrade format
    """
    try:
        # Get API key from environment
        api_key = os.getenv("BROKER_API_SECRET")
        ...
        if response_data.get("s") == "ok":
            ...
            for position in positions:
                try:
                    ...
                    # Get SilverTrade symbol - follow same approach as TradeBook implementation
                    silvertrade_symbol = None
                    try:
                        # First try with the symbol ID from sym object
                        symid_from_object = sym.get("id", "")
                        if symid_from_object:
                            silvertrade_symbol = get_oa_symbol(symid_from_object, exchange)
                            logger.info(
                                f"Symbol lookup with sym.id: {symid_from_object} -> {silvertrade_symbol}"
                            )

                        # If not found and we have the position symId, try that
                        if not silvertrade_symbol and symbol_id:
                            silvertrade_symbol = get_oa_symbol(symbol_id, "")
                            logger.info(
                                f"Symbol lookup with position.symId: {symbol_id} -> {silvertrade_symbol}"
                            )

                        # If still not found, try with exchange symbol
                        if not silvertrade_symbol:
                            silvertrade_symbol = get_oa_symbol(exchange_symbol, exchange)
                            logger.info(
                                f"Symbol lookup with exchange symbol: {exchange_symbol} -> {silvertrade_symbol}"
                            )
                    ...
                    if silvertrade_symbol:
                        final_symbol = silvertrade_symbol
                        logger.info(f"Using SilverTrade symbol: {final_symbol}")
                    else:
                        # Fallback to exchange symbol if SilverTrade symbol isn't available
                    ...
                    # Format the position data according to SilverTrade format
                    # Removing tradingsymbol field as requested
                    transformed_position = {
                        "symbol": final_symbol,  # Use final symbol (SilverTrade or fallback)
                        "exchange": exchange,
                    ...
                    logger.debug(f"Position transformed: {tradingsymbol} → {silvertrade_symbol}")
            ...
            # Return in SilverTrade format - same pattern as orderbook and tradebook
...
def get_holdings(auth):
    ...
    Returns:
        dict: Holdings data in SilverTrade format
        {
            "data": {
                "holdings": [
                    {
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    try:
        ...
        # Check if this is already in SilverTrade format
        if positions_response.get("status") == "success" and "data" in positions_response:
            logger.info("get_open_position - Processing SilverTrade format positions")
            ...
            for position in positions:
                ...
                logger.info(
                    f"get_open_position - Checking SilverTrade position: {pos_symbol} on {pos_exch}, qty: {pos_qty}"
                )

                if pos_exch == exchange and pos_symbol == tradingsymbol and pos_qty != 0:
                    logger.info(
                        f"get_open_position - Found matching SilverTrade position: {pos_symbol} with quantity {pos_qty}"
                    )
                    return str(pos_qty)

            logger.info(
                f"get_open_position - No matching SilverTrade position found for {tradingsymbol} on {exchange}"
            )
            return "0"
    ...
    try:
        ...
        with symbol_lock:
            ...
            try:
                ...
                # Format response to match SilverTrade's expected format
                if (
                    response
...
def close_all_positions(current_api_key, auth):
    ...
    Returns:
        dict: Response with status and message in SilverTrade format
              {
    ...
    try:
        ...
        # Prepare final response in SilverTrade format
