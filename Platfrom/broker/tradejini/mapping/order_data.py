...
def map_order_data(order_data):
    ...
    if orders_data:
        for order in orders_data:
            ...
            # Map Tradejini status to SilverTrade AI status
...
def transform_order_data(orders):
    """
    Processes and modifies a list of order dictionaries into the final SilverTrade AI format.

    Parameters:
    - orders: List of orders with modified fields

    Returns:
    - Dictionary with orders in SilverTrade AI format
    """
    ...
    for order in orders:
        # Convert to SilverTrade AI format if needed
        transformed_order = {
...
def map_trade_data(trade_data):
    ...
    # Handle SilverTrade AI format with status and data fields
    if (
    ...
    if trades_data:
        for trade in trades_data:
            ...
            mapped_trade = {
                ...
                "sym_id": symbol.get("id", ""),  # Store symbol ID for SilverTrade AI lookup
            }
...
def transform_tradebook_data(trades):
    """
    Transforms mapped trade data to SilverTrade AI format.

    Args:
        trades: List of mapped trade dictionaries or raw API response

    Returns:
        dict: Trade book data in SilverTrade AI format with {'data': [...], 'status': 'success'}
    """
    logger.debug(f"transform_tradebook_data - Input trades type: {type(trades)}")

    # Check if already in SilverTrade AI format
    if isinstance(trades, dict) and "status" in trades and "data" in trades:
        logger.debug("transform_tradebook_data - Already in SilverTrade AI format")
    ...
    for trade in trades:
        ...
        # Get SilverTrade AI symbol if possible
        try:
            silvertrade_symbol = get_oa_symbol(symbol=sym_id, exchange=exchange)
        ...
        # Create transformed trade - match SilverTrade AI format exactly
        transformed_trade = {
            "action": action,
...
def map_position_data(position_data):
    ...
    for position in position_data:
        try:
            ...
            # Get SilverTrade AI symbol - follow same approach as the main implementation
            silvertrade_symbol = None
            try:
            ...
            if silvertrade_symbol:
                ...
                logger.info(f"Using SilverTrade AI symbol: {final_symbol}")
            else:
                # Fallback to exchange symbol if SilverTrade AI symbol isn't available
...
def transform_positions_data(positions_data):
    """
    Transform mapped position data to SilverTrade AI format.
    DEPRECATED: This function is kept for backward compatibility only.
...
def transform_holdings_data(holdings_data):
    """
    Transforms Tradejini holdings data to SilverTrade AI format.

    Args:
        holdings_data (list): List of holdings dictionaries from TradeJini API

    Returns:
        dict: Holdings data in SilverTrade AI format
        {
            "data": {
                "holdings": [
                    {
