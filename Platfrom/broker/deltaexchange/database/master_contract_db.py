...
def init_db():
    ...
    try:
        ...
        logger.error(
            ...
            f"the column is added. Run: sqlite3 db/silvertrade.db "
...
def _to_canonical_symbol(delta_symbol: str, instrument_type: str, expiry: str) -> str:
    """
    Convert a Delta Exchange native symbol to the SilverTrade AI canonical CRYPTO format.
    ...
    Returns:
        SilverTrade AI canonical symbol string.
    """
...
# Maps Delta Exchange contract_type values to SilverTrade AI instrument type codes
CONTRACT_TYPE_MAP = {
...
def process_delta_products(products):
    ...
    SilverTrade AI SymToken schema.  Only live + operational products are included.

    Field mapping (from GET /v2/products response):
        token          ← id                    (int → str)
        brsymbol       ← symbol                (Delta-native, e.g. "C-BTC-80000-280225")
        symbol         ← canonical             (SilverTrade format, e.g. "BTC28FEB2580000CE")
        name           ← description
        exchange       ← "CRYPTO"              (SilverTrade exchange abstraction)
    ...
    for p in products:
        ...
        # Build SilverTrade AI canonical symbol (exchange = CRYPTO, broker-agnostic format)
        canonical_symbol = _to_canonical_symbol(symbol_str, instrument_type, expiry)

        rows.append(
            {
                "token": str(p["id"]),
                "symbol": canonical_symbol,  # SilverTrade AI canonical (e.g. BTC28FEB2580000CE)
                "brsymbol": symbol_str,  # Delta-native (e.g. C-BTC-80000-280225)
                "name": p.get("description", symbol_str),
                "exchange": "CRYPTO",  # SilverTrade AI exchange abstraction
                "brexchange": "DELTAIN",  # Broker identifier (Delta Exchange India)
                "expiry": expiry,
                "strike": strike_val,
                "lotsize": lotsize,
