# Mapping SilverTrade AI API Request to Delta Exchange API Parameters
...
def transform_data(data, token):
    """
    Transforms the SilverTrade AI API request structure to Delta Exchange POST /v2/orders payload.
...
def transform_modify_order_data(data):
    """
    Transforms SilverTrade AI modify order data to Delta Exchange PUT /v2/orders payload.
    ...
    SilverTrade AI pricetype mapping:
...
def map_order_type(pricetype):
    """Maps SilverTrade AI pricetype to Delta Exchange order_type string."""
    mapping = {
        "MARKET": "market_order",
        "LIMIT": "limit_order",
        "SL": "limit_order",
...
def reverse_map_product_type(br_product):
    """
    Maps Delta Exchange position category back to SilverTrade AI product type.
...
def map_exchange(br_exchange):
    """
    Maps a Delta Exchange broker exchange field back to SilverTrade AI exchange code.
...
def map_exchange_type(exchange):
    """
    Maps an SilverTrade AI exchange code to the Delta Exchange context.
