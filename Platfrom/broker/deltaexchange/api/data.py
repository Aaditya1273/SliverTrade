...
def _get_ticker(symbol: str) -> dict:
    ...
    for attempt in range(_MAX_RETRIES + 1):
        try:
            resp = request_with_circuit_breaker(
                "GET", url, headers={"Accept": "application/json"}, timeout=15.0
            )
            break  # success
...
class BrokerData:
    ...
    # Delta Exchange supported candle resolutions mapped from SilverTrade interval codes.
    ...
    def __init__(self, auth_token: str):
        """Initialise with the api_key stored in the SilverTrade auth DB."""
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        Field mapping (ticker result → SilverTrade):
    ...
    def get_option_chain(
        ...
        Args:
            ...
            exchange:   SilverTrade exchange code.  ``"CRYPTO"`` for all Delta
    ...
    def get_intervals(self) -> list:
        """
        Return the list of supported SilverTrade interval codes for Delta Exchange.
        """
    ...
    def _get_br_symbol(self, symbol: str, exchange: str) -> str:
        """
        Resolve SilverTrade symbol → Delta Exchange contract symbol (brsymbol).
