...
def get_api_response(endpoint, auth, method="GET", payload="", params=None):
    ...
    Args:
        ...
        auth:     api_key (BROKER_API_KEY stored in SilverTrade AI DB after login)
        ...
        try:
            ...
            if m == "GET":
                ...
                response = request_with_circuit_breaker(
                    "DELETE", url, headers=headers, content=body
                )
            else:
...
def get_positions(auth):
    ...
    appear in the SilverTrade AI position book alongside derivative positions.
    """
    positions = []
...
def close_all_positions(current_api_key, auth):
    ...
    for pos in positions:
        ...
        # Resolve SilverTrade AI symbol from DB.
