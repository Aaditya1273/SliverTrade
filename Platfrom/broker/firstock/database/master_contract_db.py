...
def process_firstock_nse_data(output_path):
    """
    Processes the Firstock NSE data (NSE_symbols.csv) to generate SilverTrade symbols.
    ...
    # Apply transformation for SilverTrade symbols
    def get_silvertrade_symbol(broker_symbol):
    ...
    df["symbol"] = df["brsymbol"].apply(get_silvertrade_symbol)

    # For index rows, normalize to SilverTrade-standard symbol using the
    ...
    if index_mask.any():
        df.loc[index_mask, "symbol"] = df.loc[index_mask].apply(
            lambda r: map_to_silvertrade_index_symbol(r["brsymbol"], r.get("name", ""), "NSE"),
            axis=1,
        )
...
def process_firstock_nfo_data(output_path):
    """
    Processes the Firstock NFO data (NFO_symbols.csv) to generate SilverTrade symbols.
...
def process_firstock_bse_data(output_path):
    """
    Processes the Firstock BSE data (BSE_symbols.csv) to generate SilverTrade symbols.

    Indices (SENSEX, SENSEX50, etc.) are identified by empty ISIN + zero
    TickSize + zero FreezeQty (same heuristic as NSE) and routed to
    exchange='BSE_INDEX' with SilverTrade-standard symbol normalization.
    ...
    # For index rows, normalize to SilverTrade-standard symbol (e.g. SENSEX,
    # SENSEX50, BANKEX). Unlisted indices fall through unchanged.
    index_mask = df["is_index"]
    if index_mask.any():
        df.loc[index_mask, "symbol"] = df.loc[index_mask].apply(
            lambda r: map_to_silvertrade_index_symbol(r["brsymbol"], r.get("name", ""), "BSE"),
            axis=1,
        )
...
def process_firstock_bfo_data(output_path):
    """
    Processes the Firstock BFO data (BFO_symbols.csv) to generate SilverTrade symbols.
...
# SilverTrade standard index symbols per symbol_Openalgo.md.
# Matching any fetched idxname/tradingSymbol against these (case- and
# whitespace-insensitive) normalizes to the SilverTrade format; unmatched
# indices pass through unchanged so new/custom indices still get stored.
SILVERTRADE_NSE_INDICES = {
    "NIFTY",
    "NIFTYNXT50",
    "FINNIFTY",
    "BANKNIFTY",
...
# Explicit aliases for broker/exchange names that diverge from the SilverTrade
...
# Pre-computed normalized lookup dicts: normalized_key -> canonical SilverTrade symbol.
# Built once at module load so tolerant matching is a single dict lookup.
_NSE_INDEX_NORMALIZED_LOOKUP = {_normalize_index_key(s): s for s in SILVERTRADE_NSE_INDICES}
_BSE_INDEX_NORMALIZED_LOOKUP = {_normalize_index_key(s): s for s in SILVERTRADE_BSE_INDICES}
...
def _basic_index_cleanup(value):
    ...
    the stored symbol is a valid SilverTrade-format token.
    """
    if not value:
        return ""
...
def map_to_silvertrade_index_symbol(trading_symbol, idxname, br_exchange):
    """
    Resolve the SilverTrade-standard index symbol for a fetched Firstock index.

    Match order:
      1. tradingSymbol is already an SilverTrade canonical (NIFTY, SENSEX, ...)
      2. tradingSymbol/idxname normalizes to an explicit alias (e.g. BSEIT)
      3. tradingSymbol/idxname normalizes to a canonical SilverTrade symbol
...
def fetch_firstock_indices():
    ...
    try:
        ...
        for item in indices:
            ...
            silvertrade_symbol = map_to_silvertrade_index_symbol(
