...
def process_firstock_nse_data(output_path):
    ...
    if index_mask.any():
        df.loc[index_mask, "symbol"] = df.loc[index_mask].apply(
            lambda r: map_to_silvertrade_index_symbol(
...
_NSE_INDEX_NORMALIZED_LOOKUP = {
    _normalize_index_key(s): s for s in SILVERTRADE_NSE_INDICES
}
_BSE_INDEX_NORMALIZED_LOOKUP = {
    _normalize_index_key(s): s for s in SILVERTRADE_BSE_INDICES
}
