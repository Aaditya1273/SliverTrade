...
def transform_data(data, token, auth_token=None):
    """
    Transforms the SilverTrade AI API request to Firstock's V1 /placeOrder structure.

    For MARKET and SL-M orders, applies client-side Market Price Protection
    (MPP): fetch LTP, compute a protected price via the SilverTrade AI MPP slab,
    ...
    Args:
        data: Order data dictionary (SilverTrade format).
...
def transform_modify_order_data(data, token, auth_token=None):
    ...
    used by transform_data so the price is owned by SilverTrade AI. When we can't
    ...
    Expects data["symbol"] to be in SilverTrade AI format; the broker symbol is
...
def map_order_type(pricetype):
    """
    Maps the SilverTrade AI pricetype to Firstock's order type.
    """
...
def map_product_type(product):
    """
    Maps the SilverTrade AI product type to Firstock's product type.
    """
...
def reverse_map_product_type(product):
    """
    Maps Firstock's product type to SilverTrade AI product type.
    """
