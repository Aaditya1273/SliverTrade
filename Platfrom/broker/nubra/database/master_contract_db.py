...
def download_nubra_instruments(output_path):
    ...
    headers = {"Authorization": f"Bearer {auth_token}", "x-device-id": "SILVERTRADE"}
...
def process_nubra_json(path):
    ...
    and converts it into SilverTrade AI master contract format.
    ...
    - Symbol format follows SilverTrade AI F&O spec:
    ...
    # SilverTrade AI exchange mapping
    ...
    # ---------------- Symbol Construction (SilverTrade format) ----------------
...
def process_nubra_indexes(path):
    """
    Processes the Nubra index CSV file to fit the SilverTrade AI database schema.
    ...
    Output format follows SilverTrade AI convention:
    - exchange = NSE_INDEX or BSE_INDEX
    - brexchange = NSE or BSE (original exchange)
    - instrumenttype = 'INDEX'
    - symbol = Standardized SilverTrade AI format (NIFTY, BANKNIFTY, SENSEX, etc.)
    ...
    # Map CSV columns to SilverTrade AI database schema
    df = df.rename(
        columns={
            "EXCHANGE": "brexchange",
            "INDEX_SYMBOL": "symbol",
    ...
    # Map to SilverTrade AI index exchange format
    ...
    # Common Index Symbol Formats - map Nubra INDEX_SYMBOL to SilverTrade AI standard
    # Reference: SilverTrade AI symbols.md
    nubra_to_silvertrade_index = {
        # ---- NSE Indexes ----
        "INDIA_VIX": "INDIAVIX",
    ...
    df["symbol"] = df["symbol"].replace(nubra_to_silvertrade_index)
