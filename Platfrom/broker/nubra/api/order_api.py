...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    """
    # Convert Trading Symbol from SilverTrade AI Format to Broker Format Before Search in OpenPosition
...
def place_order_api(data, auth):
    ...
    # Transform SilverTrade AI data to Nubra format
    ...
    if response.status_code in [200, 201] and "order_id" in response_data:
        ...
        # Normalize response format for SilverTrade AI compatibility
        response_data["status"] = True
        response_data["data"] = {"orderid": orderid}
        # SilverTrade AI service layer expects status 200 for success
        response.status = 200
    else:
        orderid = None
...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions:
        ...
        # Try to get SilverTrade AI symbol from database using ref_id
