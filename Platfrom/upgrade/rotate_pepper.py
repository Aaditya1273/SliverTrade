#!/usr/bin/env python3
"""
SilverTrade PEPPER Rotation Migration (DESTRUCTIVE)
=================================================
...
Pre-flight:
  1. Stop SilverTrade AI (kill the running process / systemctl stop silvertrade).
  2. Back up db/silvertrade.db (the script also creates a backup, but
     belt-and-braces).
  3. Make sure no other writer is touching the DB.

Post-flight:
  1. Restart SilverTrade AI.
...
Columns rotated:
  auth_db Fernet (PBKDF2-SHA256, salt=b"silvertree_static_salt"):
    - auth.auth
    - auth.feed_token
...
def _telegram_db_fernet(pepper: str) -> Fernet:
    ...
    salt = os.getenv("TELEGRAM_KEY_SALT", "telegram-silvertrade-salt").encode()
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32,
        salt=salt,
...
def _resolve_db_path() -> str:
    ...
    db_url = os.getenv("DATABASE_URL", "sqlite:///db/silvertrade.db")
...
def main():
    ...
    print()
    print("=" * 72)
    print("  SilverTrade AI PEPPER Rotation Migration")
    print("=" * 72)
    ...
    if not args.dry_run:
        ...
        backup_path = os.path.join(backup_dir, f"silvertrade.db.before-rotate-pepper-{ts}")
    ...
    print()
    print("  Next steps:")
    print("    1. Restart SilverTrade AI: uv run app.py  (or systemctl restart …)")
