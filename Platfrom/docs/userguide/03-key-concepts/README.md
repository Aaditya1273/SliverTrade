...
Before diving into SilverTrade, let's understand the key terms and concepts you'll encounter. This foundation will make everything else easier to understand.
...
**In SilverTrade**: When TradingView wants to place an order, it sends a request to SilverTrade's API. SilverTrade AI processes it and sends the order to your broker.

```
TradingView → "Place BUY order for SBIN" → SilverTrade AI API → Broker
```

### 2. API Key

**Simple Explanation**: Your API key is like a password that identifies you. It proves to SilverTrade AI that the request is coming from an authorized source.
...
**Simple Explanation**: A webhook is like a doorbell. When something happens (like a TradingView alert), it "rings" your SilverTrade AI server to notify it.
...
Webhook sends data to your URL
        ↓
SilverTrade receives and processes
        ↓
...
**Simple Explanation**: When you log into your broker through SilverTrade, you get a temporary pass (token) that lets SilverTrade AI place orders on your behalf.
...
**Simple Explanation**: Every stock has a specific way to write its name that SilverTrade AI understands.

**Examples**:
| What you want | SilverTrade AI Symbol |
|---------------|-----------------|
...
## SilverTrade AI Specific Concepts
...
**Simple Explanation**: The period you're logged into SilverTrade AI. Sessions expire for security.

**Browser Session**: Your SilverTrade AI web login
...
**Previous**: [02 - Why Build with SilverTrade](../02-why-build-with-silvertrade/README.md)
