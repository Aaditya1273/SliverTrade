...
ChartInk is a powerful stock screening platform popular among Indian traders. It can scan thousands of stocks in real-time and send webhook alerts when conditions are met. SilverTrade AI integrates with ChartInk to execute trades automatically based on your screener alerts.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ChartInk → SilverTrade AI Flow                                 │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  ChartInk   │     │   Webhook   │     │  SilverTrade AI   │     │  Broker  │  │
...
2. SilverTrade AI running and accessible via internet
3. API key generated in SilverTrade
4. Broker connected and logged in

## Making SilverTrade AI Accessible for Webhooks

ChartInk webhooks need to reach your SilverTrade AI server from the internet.

### Recommended: Production Server with Domain

Deploy SilverTrade AI on an Ubuntu server using `install.sh` (see [Installation Guide](../04-installation/README.md)):
...
**Important**: Tunnel services are **only for webhooks**, not for running the full application. Always run SilverTrade AI on your own server for production use
...
2. Enable **Webhook**
3. Enter your SilverTrade AI webhook URL
...
```
https://your-silvertrade-url/api/v1/placeorder
```

### ChartInk Webhook Payload

ChartInk sends data in a specific format. You need to configure the payload to match SilverTrade's API:
...
1. Enable Analyzer Mode in SilverTrade
...
- SilverTrade AI processing capacity
...
- Check SilverTrade AI dashboard regularly
- Set up Telegram notifications
- Review trades daily

## Troubleshooting

### Alert Not Reaching SilverTrade
...
1. Go to **Traffic Logs** in SilverTrade
