...
When deploying SilverTrade AI using `install.sh` on Ubuntu server, most security measures are **automatically configured**. This document covers what's already done and what remains for you.
...
**When**: First login to SilverTrade
...
**How**:
```bash
# View SilverTrade AI logs
sudo journalctl -u silvertrade-yourdomain-broker -f
...
# Update SilverTrade AI dependencies
cd /var/python/silvertrade-flask/*/silvertrade
...
```bash
# Check logs
sudo journalctl -u silvertrade-yourdomain-broker -n 50
...
sudo chown -R www-data:www-data /var/python/silvertrade-flask/*/
sudo chmod -R 755 /var/python/silvertrade-flask/*/
```
...
4. **Review logs**:
   ```bash
   sudo journalctl -u silvertrade-* --since "24 hours ago"
