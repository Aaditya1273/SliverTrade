...
silvertrade on Docker, Ubuntu, Debian, RHEL/CentOS, Fedora, or Arch.
...
1. Parse `symbol`, `exchange`, `chart_type`, `interval`, and `days` from the
   user's message.
2. Fetch OHLCV history via the SilverTrade AI Python SDK
...
silvertrade pins `kaleido==1.2.0` in `pyproject.toml`. Kaleido had a major
architectural change between v0.2.x and v1.x, and the switchover is the
single most common reason new silvertrade installs see `/chart` silently fail:
...
```bash
docker exec silvertrade-web /usr/bin/chromium --version
...
```bash
# Docker
docker exec silvertrade-web /app/.venv/bin/python -c '
...
rest of silvertrade still installs and everything except `/chart` works:
...
silvertrade container itself is what runs Chromium, and the container gets it
...
```bash
sudo systemctl restart silvertrade   # or whichever unit name install.sh created
```
...
3. **`loop.run_in_executor(None, ...)` does not save you under eventlet.**
   ...
   silvertrade runs gunicorn with `--worker-class eventlet -w 1` — both in Docker
...
1. **Container/host logs first.**
   ```bash
   # Docker
   docker logs silvertrade-web --since 5m | grep -E "telegram_bot_service|chart|kaleido|chromium"

   # Bare metal
   sudo journalctl -u silvertrade --since "5 minutes ago" | grep -E "telegram_bot_service|chart|kaleido|chromium"
   ```
...
- **Moving off eventlet.** The gunicorn maintainers have deprecated the
  ...
  backward compatibility with how silvertrade integrates Flask-SocketIO). If
  silvertrade ever migrates to `gthread`, `gevent`, or `uvicorn`, the
