...
JSON that can be imported into the SilverTrade AI Flow Editor. It covers the
...
`instrumenttype` field which is lowercase to match the SilverTrade AI REST API.
...
| Field | Type | Default | Notes |
|---|---|---|---|
| `symbol` | string | — | SilverTrade AI symbol format. |
...
(if set). The shape returned by each maps onto the SilverTrade AI REST API's
...
Sends a Telegram message via the per-user Telegram bot configured in SilverTrade
settings.

| Field | Type | Default | Notes |
|---|---|---|---|
| `username` | string | — | SilverTrade AI login ID linked to a Telegram user. |
| `message` | string | — | Supports `{{vars}}`. |
...
Receive a webhook, then fan out: place an SilverTrade AI order **and** post a
...
```json
{
  ...
  "nodes": [
    ...
    { "id": "node_5", "type": "telegramAlert",  "position": { "x": 100, "y": 540 }, "data": { "username": "rajandran", "message": "[SilverTrade] Daily square-off done at {{time}} IST on {{date}}" } }
  ],
  "edges": [
    { "id": "e1", "source": "node_1", "target": "node_2" },
    { "id": "e2", "source": "node_2", "target": "node_3" },
...
SilverTrade standardizes broker-specific symbols to the following format. See
