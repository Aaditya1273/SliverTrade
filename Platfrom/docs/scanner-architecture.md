# SilverTrade AI Real-Time Scanner Service — Architecture & Implementation Guide
...
The service builds candles from SilverTrade's live WebSocket tick stream, bootstraps indicator state from Historify (DuckDB) at startup, and distributes candle events to multiple scanner processes via Redis Streams.
...
│  └── SilverTrade AI API: GET /history (source="Db", interval="1m")        │
...
│                                                                     │
│  SilverTrade WebSocket Proxy (port 8765)                               │
...
└─────────┬─────────┘ └──────┬────────┘ └──────┬────────┘ └──────┬──────┘
          └──────────────────┴─────────────────┴─────────────────┘
                    ...
                    │  (via SilverTrade AI API or direct   │
                    │   DuckDB write)                │
                    │                               │
...
**Purpose:** Single point of connection to SilverTrade WebSocket. Receives all ticks and publishes to Redis for downstream consumption.
...
- Authentication: `{"action": "authenticate", "apikey": "<SILVERTRADE_API_KEY>"}`
...
- SilverTrade AI supports up to 3000 symbols via connection pooling (3 connections x 1000 symbols each)
...
**Data source:** Historify (DuckDB) via SilverTrade AI REST API with `source="Db"`.
...
- Use SilverTrade's Historify write API if available
...
**SilverTrade Order API:**
- For automated execution based on scan results
- Route through SilverTrade's PlaceSmartOrder API
...
2. SilverTrade AI application (WebSocket proxy must be available)
3. tick_receiver (connects to SilverTrade WebSocket, starts publishing ticks)
...
08:45  Scanner service starts
       └── Redis health check
       └── SilverTrade AI connectivity check
...
08:55  Tick receiver connects to SilverTrade WebSocket
       └── Authenticate
...
If WebSocket disconnects:
    ...
    2. Wait 1 second
    3. Reconnect to SilverTrade WebSocket
    4. Re-authenticate
...
- SilverTrade WebSocket supports 3000 symbols (connection pooling)
...
│   ├── silvertrade_client.py           # SilverTrade AI API wrapper
...
```
# requirements.txt
silvertrade                    # SilverTrade AI SDK — API and WebSocket
