...
docker run -d \
    ...
    -v "$SILVERTRADE_DIR/.env:/app/.env:ro" \
...
docker run -d \
    ...
    -v "$SILVERTRADE_DIR/strategies:/app/strategies" \
    -v "$SILVERTRADE_DIR/log:/app/log" \
    -v "$SILVERTRADE_DIR/keys:/app/keys" \                # ← NEW
    -v "$SILVERTRADE_DIR/tmp:/app/tmp" \                  # ← NEW
    -v "$SILVERTRADE_DIR/.env:/app/.env:ro" \
    --restart unless-stopped \
    "$IMAGE"
```
...
services:
  silvertrade:
    ...
    volumes:
      ...
      - silvertrade_logs:/app/logs          # ← EXTRA, UNUSED
...
volumes:
  silvertrade_db:
  silvertrade_logs:                         # ← EXTRA, UNUSED
  silvertrade_log:
  silvertrade_strategies:
  silvertrade_keys:
  # MISSING: silvertrade_tmp
```
...
- [ ] Verify numba import works: `docker exec silvertrade python -c "import numba; print('OK')"`
- [ ] Verify shared memory: `docker inspect silvertrade --format='{{.HostConfig.ShmSize}}'` should show `2147483648`
...
```bash
docker inspect silvertrade --format='{{.HostConfig.ShmSize}}'
# Should show: 2147483648 (2GB in bytes)
```

**Verify volumes:**
```bash
docker inspect silvertrade --format='{{range .Mounts}}{{.Destination}} {{end}}'
