...
SilverTrade implements centralized logging with configurable levels, file rotation, and structured output. All application logs are routed through a unified logging system stored in `logs.db` and optional file logs.
    ...
    if not logger.handlers:
        ...
        if os.getenv('LOG_TO_FILE', 'False').lower() == 'true':
            file_handler = RotatingFileHandler(
                filename=os.path.join(os.getenv('LOG_DIR', 'log'), 'silvertrade.log'),
...
│ Docs       https://docs.silvertrade.ai                         │
│                                                              │
│ Status     Ready                                             │
│                                                              │
╰──────────────────────────────────────────────────────────────╯
...
```
log/
├── silvertrade.log        # Current log file
├── silvertrade.log.1      # Previous rotation
├── silvertrade.log.2      # Older rotation
├── ...
└── silvertrade.log.14     # Oldest (based on LOG_RETENTION)
```
...
```bash
# View current log
cat log/silvertrade.log

# Follow log in real-time
tail -f log/silvertrade.log

# View last 100 lines
tail -100 log/silvertrade.log

# Search for errors
grep ERROR log/silvertrade.log
```
