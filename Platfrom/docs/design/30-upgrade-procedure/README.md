...
Guidelines for upgrading SilverTrade AI to new versions while preserving data and configurations.
...
│  □ 5. Stop running SilverTrade AI instance                                       │
...
cp db/silvertrade.db backups/$(date +%Y%m%d)/
...
SilverTrade uses automatic database initialization on startup. New tables are created automatically when the application starts - no manual migrations are required.
...
# Restore databases
cp backups/YYYYMMDD/silvertrade.db db/
...
echo "Starting SilverTrade AI upgrade..."
