# SilverTrade MCP — Tool Reference & Prompt Examples
...
- **Default strategy tag**: `python mcp` — every MCP-triggered order is tagged so you can filter MCP activity in SilverTrade AI logs and the Analyzer. Override by saying *"use strategy 'my scalper'"* in the prompt.
...
Returns the full standardized SilverTrade AI index symbol list (57 NSE + 40 BSE), rolled out uniformly across every supported broker.
...
### `get_silvertrade_version`

**Prompt:** *"What version of the silvertrade library is running?"*
...
Push a Telegram notification via the SilverTrade Telegram bot (must be configured in SilverTrade AI settings first). Supports `priority` 1–10.
...
- Start in **analyzer mode** (`analyzer_toggle True`) while you get comfortable — orders look real but never leave SilverTrade AI.
...
- For live trading, set up the SilverTrade Telegram bot and ask the assistant to *"send a Telegram alert after every order fill"* — you get a realtime feed without staring at the screen.
...
- [SilverTrade Symbol Format](./userguide/symbol-format/README.md) — how equity / future / option symbols are constructed
