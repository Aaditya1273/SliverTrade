...
This guide walks through every step required to add a new broker to SilverTrade AI. It covers the directory structure, authentication patterns, order/data APIs, symbol mapping, WebSocket streaming, master contract database, rate limiting, and all registration points across the codebase.
...
└── your_broker/
    ...
    ├── mapping/
    │   ├── transform_data.py          # SilverTrade AI ↔ broker format mapping (required)
...
```json
{
    ...
    "Plugin URI": "https://silvertrade.ai",
    "Description": "YourBroker SilverTrade AI Plugin",
    "Version": "1.0",
    "Author": "Your Name",
    "Author URI": "https://silvertrade.ai"
}
```
...
This is the critical translation layer between SilverTrade's unified format and the broker's API format.
...
def transform_data(data):
    """
    Transform SilverTrade AI order request to broker-specific format.

    Input (SilverTrade format):
        {
            "symbol": "SBIN",
            "exchange": "NSE",
            "action": "BUY",
...
Maps broker order response fields to SilverTrade's standardized format:

```python
def map_order_data(order):
    """Map broker order data to SilverTrade AI format."""
    return {
...
This module downloads the broker's instrument/symbol master file and populates the `symtoken` table, which maps SilverTrade AI symbols to broker-specific symbols.
...
def master_contract_download():
    ...
    3. Maps broker-specific symbols to SilverTrade's standardized format
...
|----------------|-------------|---------|
| `symbol` | SilverTrade AI standardized symbol | `SBIN-EQ`, `NIFTY24JAN24000CE` |
| `brsymbol` | Broker's native symbol | `SBIN`, `NIFTY24JAN24000CE` |
| `name` | Human-readable name | `State Bank of India` |
| `exchange` | SilverTrade AI exchange | `NSE`, `NFO`, `BSE`, `BFO`, `CDS`, `MCX` |
...
def map_feed_data(raw_data):
    """
    Normalize broker-specific tick data to SilverTrade's unified format.
...
SilverTrade supports five distinct authentication patterns:
