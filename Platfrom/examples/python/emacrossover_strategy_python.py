...
Run via SilverTrade's /python strategy runner:
    ...
    HOST_SERVER / WEBSOCKET_URL : inherited from SilverTrade's .env.
...
# When launched via SilverTrade's /python runner, these come from the platform:
#   SILVERTRADE_API_KEY : injected per-strategy (decrypted from DB)
#   HOST_SERVER      : inherited from SilverTrade's .env
#   WEBSOCKET_URL    : inherited from SilverTrade's .env
...
class ConfigurableEMABot:
    def __init__(self):
        ...
        print("[BOT] SilverTrade Trading Bot Started")
        ...
        if os.getenv("SILVERTRADE_STRATEGY_EXCHANGE"):
            print(
                f"[BOT] Exchange resolved from SILVERTRADE_STRATEGY_EXCHANGE "
...
if __name__ == "__main__":
    if not API_KEY or API_KEY == "silvertrade-apikey":
        print(
            "[WARNING] SILVERTRADE_API_KEY is not set in environment. "
            "Set it before running in live mode."
        )

    print("\n" + "=" * 60)
    print(" SILVERTRADE EMA STRATEGY - READY TO RUN")
    print("=" * 60)
