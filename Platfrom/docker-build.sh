#!/bin/bash
# SilverTrade Docker Build and Deployment Script
# This script builds and deploys SilverTrade with numba/llvmlite support
...
# Start container
start_container() {
    print_header "Starting SilverTrade Container"
...
health_check() {
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        docker-compose logs --tail=50 silvertrade || docker logs ${CONTAINER_NAME}
        exit 1
    fi
...
test_python_deps() {
    ...
    if docker-compose exec -T silvertrade python -c "import numba; import llvmlite; import scipy; print('SUCCESS')" 2>/dev/null | grep -q "SUCCESS"; then
    ...
    if docker-compose exec -T silvertrade python -c "
...
" 2>/dev/null | grep -q "SUCCESS"; then
    ...
    if docker-compose exec -T silvertrade bash -c "
...
show_access_info() {
    ...
    echo -e "${GREEN}✓ SilverTrade is now running${NC}\n"
    ...
    echo -e "  Shell access:     ${YELLOW}docker-compose exec silvertrade bash${NC}"
    echo -e "  Run strategy:     ${YELLOW}docker-compose exec silvertrade uv run python /app/strategies/scripts/your_script.py${NC}"
...
# Main execution
main() {
    print_header "SilverTrade Docker Build & Deploy"
