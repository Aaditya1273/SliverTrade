...
def create_checkout():
    ...
    try:
        if not billing_service.is_available():
            return jsonify({"status": "error", "message": "Billing service not available"}), 503
        ...
        if interval not in ["month", "year"]:
            return jsonify(
                {"status": "error", "message": "Invalid interval. Must be 'month' or 'year'."}
            ), 400
        ...
        if not checkout_url:
            return jsonify({"status": "error", "message": "Failed to create checkout session"}), 500

        return jsonify({"status": "success", "checkout_url": checkout_url})

    except Exception as e:
        logger.exception("Checkout creation error: %s", e)
        return jsonify({"status": "error", "message": "Failed to create checkout"}), 500
...
def customer_portal():
    ...
    try:
        ...
        if not portal_url:
            return jsonify({"status": "error", "message": "Failed to create portal session"}), 500

        return jsonify({"status": "success", "portal_url": portal_url})

    except Exception as e:
        logger.exception("Portal creation error: %s", e)
        return jsonify({"status": "error", "message": "Failed to create portal"}), 500
...
from utils.plan_limits import (
    ...
    count_user_python_strategies,
    count_user_workflows,
)
...
def get_subscription():
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "subscription": {
                    ...
                    "usage": {
                        ...
                        "last_signal_reset_at": user.last_signal_reset_at.isoformat()
                        if user.last_signal_reset_at
                        else None,
                        "signals_limit": limits["signals_per_month"],
                        "signals_remaining": (
                            None
                            if limits["signals_per_month"] is None
                            else max(
                                0, limits["signals_per_month"] - (user.signals_used_this_month or 0)
                            )
                        ),
        ...
        return jsonify({"status": "error", "message": "Failed to fetch subscription"}), 500
...
def get_usage_history():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "Failed to fetch usage history"}), 500
