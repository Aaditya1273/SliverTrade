...
def get_signals():
    ...
    return jsonify(
        {"status": "success", "count": len(filtered[:limit]), "signals": filtered[:limit]}
    )
...
def get_latest_signal():
    ...
    if not signals_history:
        return jsonify({"status": "success", "signal": None, "message": "No signals available yet"})

    return jsonify({"status": "success", "signal": signals_history[0]})
...
def generate_mock_signal():
    ...
    return jsonify({"status": "success", "signal": new_signal})
